...
всё равно проходят cull по фрустуму каскада (GPU-индирект) и вершинный шейдер. Рычаг до растра —
S14 (точный объём кулла, транскрипция `ComputeShadowCullingVolume`): **−14 %** `Pass_CSM` на том же
ракурсе. Именно поэтому у UE scissor выключен и с багом: после их cull области вне конуса и так пусты.
...
## S14. Точный объём кулла кастеров — ✅ СДЕЛАНО 2026-09-03 (транскрипция `ComputeShadowCullingVolume`)

**Зависит от:** ничего (S11 не нужен). **Эффект:** кастеры вне конуса взгляда отсекаются ДО вершинного
шейдера, на обоих путях (CPU-очередь и GPU-cull). **Риск:** тот же, что у S11 (стекло) — и UE живут с ним
по умолчанию.

### Почему UE не сужают cull до scissor-прямоугольника
Потому что их cull directional-каскада и так теснее любого прямоугольника. `ShadowSetup.cpp:5008-5022`:
цилиндр вокруг оси света радиусом сферы каскада → **`ShadowBoundsAccurate.IntersectBox`** →
`r.Shadow.RadiusThreshold`. `ShadowBoundsAccurate` строит `ComputeShadowCullingVolume`
(`DirectionalLightComponent.cpp:101`): из шести граней слайса камеры остаются смотрящие ОТ солнца
(крышки), силуэтные рёбра слайса выдавливаются вдоль света, к солнцу объём открыт. Полный ортобокс
(`CasterOuterFrustum`) у них остаётся только для НЕ-directional теней. Scissor (S11) поверх такого cull
почти ничего не режет — оттого он у них выключен и с двойным оффсетом.

### Что сделано
* `BuildCascadeCullVolume` (`Scene.cpp`) — транскрипция: грани с `n_out·toSun < 0` (UE:
  `Normal | LightDirection < 0`, `LightDirection = -GetDirection()` = К СОЛНЦУ — проверено численно в
  их системе координат, плоскости у них наружные), рёбра с разными знаками → плоскость через ребро и
  `A + toSun·|AB|`. Дельты: ориентация по центроиду слайса вместо winding'а (их `bReverseCulling` не
  нужен); слайс для cull расширен к камере на blend-полосу ПРЕДЫДУЩЕГО каскада (приёмники полосы
  S10 читают этот каскад тоже); **все шесть плоскостей ортобокса добавлены в объём** — берутся из
  уже построенного `Frustum::FromOrthoBounds`, см. ловушку ниже.
* `Frustum` получил тип `Convex` (`FromPlanes`, `PlaneCount()`, кап `kMaxPlanes = 16`);
  `render::ShadowViewFrustum` 6 → 16 плоскостей (256 байт), `shadow_cull_cs.hlsl` — литеральный цикл
  на 16, недостающие плоскости паддятся accept-all `(0,0,0,+1)`, неактивный слот — sentinel
  `(0,0,0,-1)` как раньше. Один буфер, оба cull'а — CPU и GPU видят одни байты.
* `CascadeShadowConfig::accurateCasterCull` — **дефолт ON, как у UE** (у них это даже не cvar);
  `--set=csm.accurateCull:0` = старый бокс для A/B; галка в табе CSM; readout получил столбцы
  `cull pl` (6 = бокс, 7..15 = объём), `casters` (кастеры, прошедшие CPU-cull за прошлый кадр, сквозь
  инстанс-батчи) и `leak` (записи очереди, которые отверг бы другой объём); `csm_readout.log`
  печатает и список выживших c0 с вердиктами обоих объёмов.
* Debug-ассерты: объём держит все 8 углов слайса и точки к солнцу в пределах `casterReachWS`;
  отвергает точку за дальней крышкой и точку сбоку от силуэта; **каждый объект, прошедший объём,
  проходит и бокс** (по очереди прошлого кадра).

### Ловушка: «подразумеваемые» плоскости бокса
Геометрически объём лежит внутри квадрата каскада (развёртка идёт вдоль оси Z света), и XY-грани
бокса я сначала выкинул как лишние. Для ТОЧЕЧНОГО теста они лишние; для AABB-теста (positive vertex)
— нет: у острых углов призмы бокс проходит все полуплоскости, лёжа снаружи. Без них объём пропускал
БОЛЬШЕ объектов, чем бокс. С ними объём = пересечение обоих тестов, и Debug-ассерт вложенности это
держит.

### Ловушка 2: `casters` считал записи очереди, а не кастеров
`casters` c0 давал 13 (бокс) против 17 (объём) при молчащем ассерте вложенности и `leak = 0`. Список
выживших c0 (печатается в `csm_readout.log`) показал: в box-плече первая запись — объект с
`valid=0`, в accurate-плече на его месте ШЕСТЬ пальм с `box=1 acc=1`. Это `InstancedDrawBatch`:
`BuildInstancedBatchesForBucket` начинается с `if (bucket.size() < kInstancingThreshold) return;` —
гейт по размеру ВСЕГО бакета (порог 8). Объём срезал один куст из OpaqueSimple, бакет упал до 7, и
шесть пальм остались шестью записями вместо одного батча. Ни один cull не соврал — врал счётчик.
Теперь `casters` смотрит сквозь батч (`AsInstancedDrawBatch()` — internal RTTI по образцу
`AsRenderableObject`, dynamic_cast в движке запрещён) и считает `InstanceCount()`. Заодно дамп
readout'а перенесён с кадра 9 на 600 (на 9-м уровень ещё стримится).

| каскад | casters, бокс | casters, объём | срез |
|---|---|---|---|
| c0 | 20 | 17 | −15 % |
| c1 | 113 | 80 | −29 % |
| c2 | 618 | 443 | **−28 %** |
| c3 | 618 | 618 | 0 — вся роща внутри конуса дальнего каскада |

`leak` (записи очереди, отвергаемые ДРУГИМ объёмом): в accurate-плече 0 на всех каскадах — держится
Debug-ассертом; в box-плече 1 на c0 — куст, который объём режет.

### Ловушка 3: направления плоскостей — только из ДАЛЬНЕЙ геометрии (Debug-ассерт юзера)
Юзер поймал `S14 cull volume closed toward the sun` в Debug; репро — demo.json, demo1, wind_test с
дефолтной камерой и с острова (камера теней и «вдоль солнца» — чистые). Диагностика в
session-лог, запись уровня FATAL (перед абортом: проверка, угол, худшая плоскость, дистанция) показала
дистанции −0.0014 (|p|~30) и −0.028 (|p|~300), а у плоскости ребра `n·toSun = −3e-4` — она обязана
содержать направление на солнце. Причина — не допуск и не арифметика плоскостей (сдвиг в
camera-relative координаты не изменил −0.00138 ни на йоту): **углы**. `BuildFrustumSliceCornersWS`
считает каждый угол в мировых координатах с ошибкой ~ulp(|p|) = 3e-6..3e-5 м, а ближняя грань
каскада 0 — квадрат 2 см (zNear 0.01): направление её ребра из таких углов известно лишь до 1.5e-4,
плоскость через два ближних угла и один дальний промахивается на четвёртом дальнем на
20 м × 1.5e-4 = 3 мм, а плоскость ребра, продлённая на 75 м к солнцу, — на 0.1 м.
Лечение: боковые грани и боковые рёбра проходят через апекс (камера = начало координат) и берут
направления из ЛУЧЕЙ `c[far]`; near/far-грани делят нормаль дальнего квадрата; рёбра ближней грани
параллельны рёбрам дальней и берут их направление. Ближние углы остаются только якорями — абсолютная
ошибка якоря не усиливается. У UE тот же буквальный код живёт, потому что их cascade 0 начинается с
10-сантиметровой near-плоскости камеры и считается в translated world space. Плюс гард на ребро,
почти параллельное свету (`|dir × toSun| < 1e-4·|dir|` → плоскость пропускается: объём открывается,
кастеров больше, не меньше — всегда безопасно). После правки все шесть репро-кейсов чистые.

### Ловушка 4: кросс-кадровая проверка `leak` (второй Debug-ассерт юзера, 2026-09-03)
Ассерт `S14 cull volume passed an object the ortho box rejects` при полёте по wind_test. Утечка в
одном кадре невозможна по построению (все шесть плоскостей бокса кладутся в объём без условий,
переполнение → бокс). Проверка была кросс-кадровой: `UpdateCascades` бежит ДО пересборки очередей,
выжившие в очереди отрезаны объёмом ПРОШЛОГО кадра, а тестировались боксом ТЕКУЩЕГО. Поворот
головы двигает бокс каскада (сфера слайса + снап), объект у края выпадает — «leak». Дрейф без
вращения (`--cam-fly=6,4`, 12 с, Debug) не воспроизвёл: бокс уезжает на сантиметры за кадр.

**Правило:** диагностика, сравнивающая результат cull'а с фрустумом, берёт фрустум ТОГО ЖЕ кадра.
`Scene::cascadeBoxPrev_` / `cascadeAccPrev_` хранят пару прошлого кадра; `cullLeakDbg` и список
выживших readout'а считаются от неё; динамические кастеры и сдвинутые в редакторе объекты
(`IsDynamicCaster`, `MovedThisFrame`) не считаются — их bounds могли измениться после cull'а.

**Дамп перед ассертом** (session-лог, Debug; отдельных файлов больше нет — см. AGENTS.md «Session Log»): `LEAK` — выживший не проходит бокс
прошлого кадра (настоящая дыра; печатаются оба вердикта обоих кадров и плоскости объёма прошлого
кадра), `stale` — прошёл прошлый бокс, не прошёл текущий (движение камеры; уровень Debug, лимит 32
записи на сессию, чтобы полёт не заливал лог); `LEAK` — уровень Fatal, флашится синхронно. Для headless-репро с вращением добавлен `--cam-fly-yaw=<deg/s>`.

### Валидатор GPU-cull был сломан с S3.6 — починен
`cull validation MISMATCH: 14/46 views (v0: cpu=70 gpu=0)`. Расшифровка по списку вью (добавлен в
вердикт): `gpu[v3]=67≈cpu[v0]=70, gpu[v7]=341≈cpu[v1]=351, gpu[v15]=2678≈cpu[v3]=2760` — суммы
строк 4v..4v+3 равны cpu[v] точно. GPU пишет args по ВИРТУАЛЬНЫМ группам `group*4+lod` (S3.6), а
читатель считал `valGroups_ = numMeshGroups_`. Теперь `numVirtualGroups_`; headless-запуск —
`--set=shadow.giIndirect:0` (новый флаг, аналог Ctrl-G). После починки: **`cull validation PASS: 46
views match CPU (2763 casters, 444 groups)`** — 16-плоскостной GPU-cull совпадает с CPU до штуки.

### Замер (Release, wind_test, legacy, `--profdump`, 30 с прогрева, камера теней, по два прогона)

| | `Pass_CSM` GPU | `GPU.Frame` | CPU `Pass_CSM` / `Pass_ShadowCull` |
|---|---|---|---|
| бокс (`csm.accurateCull:0`) | 0.389 / 0.377 | 2.709 / 2.705 | 0.122 / 0.045 |
| объём (дефолт) | **0.326 / 0.335 (−14 %)** | 2.656 / 2.670 | 0.122 / 0.043 |

Объём на этом ракурсе — 11 плоскостей (1 грань Far + 4 ребра + 6 бокса): солнце позади камеры, так
что призма = дальний прямоугольник слайса, протянутый к солнцу. Против S11 (−7 % при 32 % среза
площади) выигрыш вдвое: режутся вершины и сабмишн, не только филл.

### Приёмка
* Камера теней (пальмы ЗА камерой в +x, тени в кадре — ровно случай «высокий кастер вне конуса»):
  бокс→объём **0.018 %** при поле 0.016 %. Камера «пальма за краем» (`17.5,3,72.5` → `5,1,60`):
  0.042 % при поле 0.019 %. Стекло сбоку (demo.json): 0.098 % при поле 0.300 %. Всё с
  `--set=ocean.visible:0` — океан между сессиями меняет фазу и даёт −25 lum по воде при нулевой
  разнице на песке и тенях.
* Debug: ассерты молчат на камере теней.
* Наблюдение НЕ про S14: CPU-путь теней (`--set=shadow.indirect:0`) рисует на 10.14 % пикселей
  темнее GPU-пути, и ровно так же при обычном боксе — расхождение путей существовало до S14.
  Отдельная задача.

### Ловушка 5: допуск самопроверки против float на 500-метровых углах (третий Debug-ассерт юзера, 2026-09-12)
`S14 cull volume closed toward the sun`, wind_test, каскад 3, угол 7, плоскость 8, dist −0.00119
при жёстком допуске −1e-3. Плоскость 8 содержит направление на солнце (n·s ≈ 0) — силуэтная
плоскость БОКОВОГО ребра, якорь стоял в ближнем углу, а тестировался дальний угол на |p| ≈ 530 м:
ulp float32 там 6e-5 м, три произведения плюс ошибка якоря на рычаге 400 м до дальнего угла —
это и есть миллиметр; пробник «к солнцу» шагнул ещё на 75 м и добавил остаток n·s. Не геометрия
объёма (тот же вид с боксом даёт ту же картинку), а масштаб координат против фиксированного
миллиметра. Две правки в `BuildCascadeCullVolume` / самопроверке (`Scene.cpp`):
* боковые рёбра (e ≥ 8) якорятся в АПЕКСЕ (origin-относительный ноль, лежит на луче по построению):
  расстояние дальнего угла до своей плоскости — округление одного cross-произведения (~1e-4 м),
  ошибка ближнего угла больше не умножается на рычаг;
* допуск валидатора масштабируется координатами: `1e-3 + 2e-6·(|p| + шаг пробника)` — ~30 ulp,
  2.3 мм на углах каскада 3; реальный дефект объёма — дециметры, чувствительность не потеряна.
  Fatal-отчёт печатает допуск рядом с расстоянием.
Репро той же позы (`--cam-pos=-75.4642,62.3099,103.964 --cam-rot=-0.0301,0.6495,0.0257,0.7593
--set=sun.elevation:25 --set=sun.azimuth:35`, Debug) после правки: exit 0, записей S14 нет.
Правило: ассерт на расстояние до плоскости в мировых координатах — только с ulp-масштабным допуском.

### Вердикт
Работает, −14 % `Pass_CSM`, картинка нейтральна, GPU=CPU по валидатору. Дефолт ON по правилу UE.
...
## S15–S18. SDSM-режим: третий режим теней рядом с legacy-CSM и VSM — S15 + S16a/S16b СДЕЛАНЫ 2026-09-14

> ⚠ **Итог шага, прежде чем читать дальше.** SDSM на **PCF** берёт ставку целиком: на атласе
> 2048 он даёт картинку legacy@4096 при ВЧЕТВЕРО меньшей памяти (8.4 против 33.6 МБ) и +0.05 мс.
> **EVSM дефолтом OFF** — он стоит +84 МБ и +0.30 мс ради процентов картинки (S16.4/S16.6).
> И отдельно: весь первый заход сравнивал SDSM не с legacy, а с **VSM** — см. S16.1.
...
### S15 — РЕЗУЛЬТАТ (2026-09-14)

**Работает, и главный выигрыш больше, чем план обещал.** Замер `--sdsm-readout`, wind_test,
дефолтная камера уровня, frame 601 (та же строка лога несёт legacy-тексель того же кадра, так что
сравнение не требует второго артефакта):

| p | interval (m) | бокс XY (m) | тексель SDSM (mm, по осям) | legacy тексель (mm) | отношение по ХУДШЕЙ оси |
|---|---|---|---|---|---|
| 0 | 0.01–3.18 | 6.2 × 3.5 | 3.04 / 1.71 | 11.27 | **0.27×** |
| 1 | 3.18–10.67 | 21.7 × 10.2 | 10.62 / 5.01 | 39.45 | **0.27×** |
| 2 | 10.67–35.85 | 72.8 × 19.4 | 35.69 / 9.51 | 112.71 | **0.32×** |
| 3 | 35.85–300.0 | 162.4 × 51.3 | 79.59 / 25.15 | 338.12 | **0.24×** |

Критерий приёмки (2) просил ≤ 0.4× на ближнем краю и ≤ 0.5× на каскадах 1–3 — получено 0.24–0.32×
ВЕЗДЕ, и это по худшей оси; по узкой оси выигрыш до 13×. На atoll (пляжная виста,
`--cam-pos=-54.81,3.00,63.71`) те же числа: 3.29 / 6.91 / 14.52 / 19.47 мм против 11.27 / 39.45 /
112.71 / 338.12. Анизотропия бокса — не побочный эффект, а половина выигрыша: у p2 бокс 72.8 × 19.4,
то есть по одной оси он в 3.8 раза плотнее, чем дал бы квадрат.

`outOfBounds` не понадобился как отдельный счётчик: последняя партиция расширяется до far камеры по
построению (транскрипция `ComputeLogPartitionsFromZBounds`), так что пиксель без своей партиции
невозможен. `sampleCount` в ридауте — его замена: пустая партиция помечается `EMPTY` и получает
legacy-сферу, а не мусор. `cpuTailCasters=0` на обоих уровнях — грабли Anno здесь не кусаются.

**Критерий (4) НЕ выполнен: SDSM дороже.** Release, wind_test, `--profdump`, 30 с прогрева, по два
прогона на конфигурацию (разброс между прогонами 0.03–0.06 мс, так что разница ниже — настоящая):

| | Legacy | SDSM |
|---|---|---|
| `Pass_CSM` / `Pass_SdsmShadow` | 0.318 / 0.319 | 0.478 / 0.461 |
| `Pass_ShadowCull` | 0.105 / 0.104 | 0.145 / 0.139 (два прогона: основной + ре-кулл) |
| `Pass_SdsmAnalyze` | — | 0.037 / 0.036 |
| **сумма направленного света** | **0.423** | **0.650** (+0.23 мс, +54 %) |
| `GPU.Frame` | 2.933 / 2.905 | 3.126 / 3.065 (+0.18 мс, +6 %) |

Причина не в анализе — он стоит 0.037 мс, ровно как план и оценивал. Дороже стала САМА ОТРИСОВКА
глубины, и это прямое следствие того, ради чего шаг делался: тайл p0 в legacy покрывает квадрат
~22.5 м, в SDSM — бокс 6.2 × 3.5 м при том же числе текселей, так что один и тот же ствол пальмы
закрывает в тайле примерно в 13 раз больше пикселей. Растеризация платит за то, что разрешение
наконец тратится на геометрию, а не на пустой воздух. Обещание доклада (с. 25–26, «SDSM быстрее
PSSM: тесные фрустумы = бесплатный окклюжн-куллинг») у нас не срабатывает, потому что legacy-путь
эту работу уже делает сам — S14 точный объём кулла включён по умолчанию, и тайлы там не настолько
пустые, чтобы кулл по боксу что-то добавил.

### S15.1 — откуда берётся эта цена, и что её НЕ снимает (2026-09-14, продолжение)

Четыре прогона-зонда, каждый меняет ровно одну вещь (wind_test, `Pass_SdsmShadow`, мс):

| конфигурация | цена | вывод |
|---|---|---|
| SDSM, дефолт | 0.478 | — |
| `sdsm.minScaleOverSphere:1.0` (бокс зажат до legacy-сферы) | **0.683** | ШИРЕ бокс — ДОРОЖЕ. Гипотеза «платим за заливку тесного тайла» опровергнута: цена растёт с ОБЪЁМОМ КУЛЛА, а не с плотностью текселей |
| `csm.casterReach:20` (слой к солнцу 150 → 20 м) | 0.440 | −0.04, мелочь |
| `csm.pancakeCasters:0` | 0.447 | −0.03, мелочь |

И контрольный зонд с другой стороны: **legacy с выключённым S14** (`csm.accurateCull:0`) стоит
**0.363** против 0.318 — то есть точный объём кулла даёт legacy 0.045 мс. Значит из разрыва
SDSM↔legacy примерно **треть** объясняется тем, что у SDSM кулл — простой AABB в light-space,
а у legacy — призма среза, выдавленная к солнцу (S14). Остаток (~0.08 мс) — то, что бокс по
СЕМПЛАМ всё равно шире клина камеры: у p3 бокс 162×51 м, а срез 35–300 м — узкий клин внутри него.

Заодно проверена и отвергнута гипотеза «пермутация VS грузит матрицу партиции на каждую вершину»:
дизассемблер даёт +6 инструкций и +5 buffer-load'ов против legacy. Пермутацию всё равно убрали —
но по другой причине, см. ниже.

### S15.2 — GPU пишет КОНСТАНТНЫЕ БУФЕРЫ, CPU биндит их адрес (2026-09-14)

Приём, который убрал из шага все пермутации сразу: **root CBV в D3D12 принимает GPU virtual
address**, значит константный буфер, который посчитал compute-шейдер, биндится ровно как
загруженный с CPU. `Finalize` дописывает два готовых блока:

* на партицию — 256-байтный `PerViewCB` (viewProj + ветровой хвост + bias). `Pass_SdsmShadow`
  биндит `SdsmShadows::ViewCbAddress(p)` в b1 — и **пермутация `SDSM_PARTITION` шейдера глубины
  удалена целиком**: два PSO, переставленная таблица дескрипторов в masked-варианте и 16 байт на
  КАЖДОМ PerView в движке (включая gbuffer) — всё это было платой за значение, которое D3D12
  спокойно берёт как CBV, раз на партицию идёт свой ExecuteIndirect;
* один `CascadeHzbCB` (560 байт): prev-матрицы из копии партиций ПРОШЛОГО кадра, текущие — из
  этого, обе уже с z-флипом (`SdsmFlipZ`, транскрипция `CascadeHzb::FlipZ`). Поэтому HZB-кулл
  ниже не потребовал ни строчки в `shadow_cull_cs.hlsl` и `shadow_cull_post_cs.hlsl`.

Выигрыш по времени от снятия пермутации: `Pass_SdsmShadow` 0.478 → ~0.44. Главное тут не 0.04 мс,
а то, что мода стала ПРОЩЕ: ноль пермутаций вне собственного анализа.

### S15.3 — HZB-кулл для SDSM: сделан, корректен, на наших сценах НЕ ОКУПАЕТСЯ (2026-09-14)

Просьба юзера. Вся машинерия S5b переиспользована как есть — те же пирамиды, тот же отложенный
список, та же пара pass-B; отличались только матрицы, и они теперь приходят GPU-блоком. Пассы —
те же `Main_CsmHzb` / `Main_ShadowCullPost` / `Main_CSMPost` (те же ресурсы, та же работа),
зарегистрированные в SDSM-цепочке. Ручка общая с legacy: `csm.hzbCull` (дефолт **выкл**).

Замер, по два прогона на арм:

| | HZB выкл | HZB вкл |
|---|---|---|
| `Pass_SdsmShadow` | 0.446 / 0.436 | 0.442 / 0.451 |
| `Pass_CsmHzb` (сборка пирамид) | — | **0.350 / 0.352** |
| `Pass_ShadowCullPost` | — | 0.007 / 0.006 |
| `GPU.Frame` | 3.102 / 3.102 | 3.262 / 3.275 |

**Отрисовка не подешевела вообще**, а пирамиды стоят 0.35 мс → **+0.17 мс на кадр**. Почему —
видно в ридауте: `deferred 0 / 0 / 0 / 1`. Пальмовая роща на плоском острове при высоковатом
солнце **не перекрывает сама себя в теневом тайле**: окклюдить в этих тайлах нечего. Ровно
поэтому legacy возит тот же S5b выключенным по умолчанию.

Вывод: функция есть, проверена (`deferred 1 → drawnB 0`, то есть отложенный кастер честно вырезан,
а не потерян), включается одной ручкой — и ОСТАЁТСЯ ВЫКЛЮЧЕННОЙ. Её стоит вспомнить на сцене с
настоящей теневой глубиной (город, лес на склоне), а не на атолле.

### S15.4 — точный объём кулла: сделан, корректен, НИЧЕГО НЕ ДАЁТ (2026-09-14)

`BuildCascadeCullVolume` (= UE `ComputeShadowCullingVolume`) транскрибирован в `Finalize`
(`SdsmCullVolumePlanes`), вместе с обоими правилами точности оригинала: всё считается
ОТНОСИТЕЛЬНО КАМЕРЫ, а направления берутся только из ДАЛЬНЕЙ геометрии (боковые грани и рёбра
идут через апекс и берут направление из угловых ЛУЧЕЙ; рёбра ближнего квада занимают направление
у параллельного дальнего). Срез расширен назад на blend-полосу предыдущей партиции (S10), как в
legacy. Ручка общая — `csm.accurateCull`. Планы: до 4 граней + до 6 рёбер + 6 граней бокса ≤ 16.

Добавлена GPU-самопроверка — двойник Debug-ассертов legacy: все 8 углов среза пробиваются об
итоговые планы с допуском, масштабирующимся по координатам, и промах поднимает флаг
`VOLUME-LEAK` в ридауте. **За все прогоны не поднялся ни разу** — объём корректен.

Замер, по два прогона на арм:

| | объём ВЫКЛ (бокс) | объём ВКЛ |
|---|---|---|
| `Pass_SdsmShadow` | 0.435 / 0.441 | 0.448 / 0.436 |
| `GPU.Frame` | 3.050 / 3.054 | 3.037 / 3.046 |

**Ноль. Разница внутри разброса прогонов.** И это — опровержение рекомендации из S15.3 выше
(«замеренный потолок 0.045 мс, треть разрыва»): 0.045 мс призма даёт LEGACY, потому что у legacy
бокс — квадрат вокруг СФЕРЫ среза, и призма режет от него огромный кусок. У SDSM бокс уже подогнан
по видимым СЕМПЛАМ, то есть практически совпадает с XY-силуэтом призмы — резать нечего.
Перенос рычага с одной моды на другую по абсолютной цифре оказался неверным ходом.

### Итог по цене: разрыв объяснён, и это не дефект

Три независимых рычага проверены и НЕ помогли (HZB-кулл, точный объём, панкейкинг/reach — все
≈0), один помог мало (снятие пермутации VS, −0.04), и один ухудшает (широкий бокс, +0.24).
Остаётся единственное объяснение, согласованное со всеми пятью замерами: **растеризуется ровно то,
что и должно.** Legacy тратит бóльшую часть тайла на пустой воздух — те же кастеры покрывают в нём
малую долю текселей; SDSM подогнал тайл по геометрии, и те же кастеры заполняют его почти целиком.
+0.12 мс — это цена того, что разрешение наконец тратится на геометрию, а не дефект, который
где-то ждёт починки.

Отсюда единственный оставшийся рычаг и есть ставка S16: **тайлы 1K вместо 2K** — вчетверо меньше
заливки и памяти, а тексель при нынешних 0.24–0.32× от legacy станет 0.48–0.64×, то есть всё ещё
вдвое лучше legacy. Плюс S17 «число партиций по питчу»: в top-down три тайла из четырёх не нужны.

### S15.5 — разрешение атласа вынесено в настройку (2026-09-14) — ЦИФРЫ СНЯТЫ, см. S16.1

> ⚠ Ручка сделана и работает. А вот колонка «legacy» в таблице ниже — это на самом деле **VSM**
> (`--shadow-mode=csm` тихо фолбэчил туда, S16.1). Актуальные цифры — в таблицах S16.1;
> вывод «вчетверо меньший атлас при 1.84× плотности» НЕ подтвердился: на равном атласе у мод паритет.

Атлас перестал быть литералом: `render::g_csmAtlasRes` (512..8192, степени двойки),
`--set=csm.atlasRes:N`, комбо в CSM-вкладке (с подписью «tile / content / МБ»), сохраняется в
`graphics_settings.json` как `shadows.legacyCsm.atlasResolution`. Смена РЕАЛЛОЦИРУЕТ атлас, поэтому
она реконсилится на GPU-idle рядом с переключением режима (`Scene::ReconcileShadowMode`), а не
применяется там, где подвинули ползунок. Ручка ОБЩАЯ для legacy и SDSM — они рисуют в один атлас,
и только поэтому их вообще можно сравнивать.

Первый же замер этой ручкой закрывает главный вопрос шага. **SDSM на 2048 против legacy на 4096:**

| | legacy @ 4096 | SDSM @ 4096 | **SDSM @ 2048** |
|---|---|---|---|
| тексель p0 (худшая ось) | 11.27 мм | 3.04 мм | **6.12 мм** |
| `Pass_SdsmShadow` / `Pass_CSM` | 0.318 | 0.446 | 0.371 / 0.339 |
| `GPU.Frame` | 2.933 / 2.905 | 3.102 | **2.918 / 2.849** |
| память атласа (R16) | 33.5 МБ | 33.5 МБ | **8.4 МБ** |

То есть SDSM при ВЧЕТВЕРО меньшем атласе даёт тени **в 1.84 раза плотнее**, чем legacy на полном,
и при этом `GPU.Frame` НЕ ХУЖЕ legacy (2.88 против 2.92 в среднем по двум прогонам каждого).
Критерий (4) в такой постановке выполнен — а +0.12 мс на 4096 были не регрессом, а просто платой
за качество, которого никто не просил: на 4096 SDSM даёт тексель 0.27× от legacy, то есть втрое
больше нужного.

**Это меняет вывод по всему S15.** Правильное сравнение мод — не «тот же атлас», а «та же цена
кадра»: при равном `GPU.Frame` SDSM почти вдвое плотнее. Ставку S16 («1K тайлы бьют legacy 2K»)
можно считать подтверждённой ещё до EVSM; EVSM теперь нужен не чтобы её выиграть, а чтобы убрать
ползание кромок, которого в SDSM больше (снапа нет по построению).

Заодно удалена ручка `sdsm.analyzeFullRes`: она была объявлена, прокинута в `--set` и **никем не
читалась** — редукция всегда идёт по всем пикселям. Инертный контрол убран, а не задизейблен;
причина оставить полное разрешение записана в шапке `ShadowSettings.h` (редукция по разрежённой
сетке теряет тонкий объект, а именно он и подтягивает ближнюю границу партиции; вся аналитика
стоит 0.037 мс, так что выбирать не из чего).

### Побочная находка гейта: компаратор барьеров поймал дыру, которую GBV не видит

`--barrier-cmp` дал `pass=30 SKIPPED (body performed nothing - registration unverified)` —
`Main_SdsmCull` объявлял переходы args/visible-list и на кадрах, где набор кастеров ещё пуст,
выходил из тела РАНЬШЕ своих `EmitPoint`. Скомпилированные барьеры этой точки тогда не
записываются, а модель компиляции считает, что записались: молчаливое рассогласование, которое
проявится у любого позднего читателя этих буферов. GBV этого не видит — состояния формально
согласованы внутри кадра.

Починено во всех телах SDSM по правилу legacy `RecordCullPost`: **точка эмитится всегда, ранний
выход пропускает только РАБОТУ между точками** (`RecordDirectionalCull` — и пустой набор, и отказ
аллокации CB; `RecordAnalyze` — отсутствующий depth SRV; `Pass_SdsmShadow` — нулевые указатели).
После правки `logs/barrier_diag.log` не создаётся вовсе.

**Чего доказать НЕ удалось: пиксельный контроль legacy/VSM.** Правила 1 и 2 требуют бит-в-бит при
переключении режима, и штатный `--shot` этого измерить не может: задержка снимка задана ВРЕМЕНЕМ,
так что три прогона одной и той же конфигурации сняли кадры 2341 / 2303 / 2262 — до 79 кадров
врозь. Всё, что копится по кадрам (пена и surf-sim океана, темпоральный SSR, GTAO, облака), успело
уехать на разное расстояние, и пол шума на wind_test вышел **9.6–12.9 % пикселей при худшей дельте
~170** — при том, что A/B legacy дал 10.9 % в одном заходе и 76 % в другом. Метрика меряет разброс
прогонов, а не правку (обещанные AGENTS.md 0.05–0.4 % были, судя по всему, измерены на уровне без
surf-sim). `--wind-freeze`, `--dlss=off`, `--set=exposure.autoExposure:0` и `--set=ssr.temporal:0`
пол не уронили.

Что вместо этого есть:
* **структурно**: при `decisions_.sdsmActive == false` три SDSM-пасса вообще не регистрируются в
  графе, `frame_->sdsm` = null, `csmSdsmPartitions` / `csmSdsmParams.x` = 0, `CsmApplySdsm` не
  вызывается ни разу, а правка загрузки каскадных фрустумов сидит под `render::SdsmActive()`.
  Правки общего кода аддитивны: хвосты CB, по одному дескриптору в трёх таблицах (не читается),
  две новые пермутации PSO;
* **детерминированные гейты**, все зелёные И ИМЕННО В РЕЖИМЕ SDSM (то есть на коде, который
  исполняется): Debug `--gbv --barrier-cmp` — ноль ошибок валидации, кадр снят; Release
  `--scene-stress=4 --barrier-cmp` — `verdict: CLEAN`, `barriers: emit enhanced=2559 legacy=0`;
  компаратор барьеров молчит (нового `logs/barrier_diag.log` нет); `cull validation PASS: 46 views
  + camera match CPU` — то есть ОСНОВНОЙ кулл правкой не задет; три конфига собираются,
  `check_shaders` 95/95, `check_logging` 0 findings.

**ОТКРЫТО, до S16:** детерминированный пиксельный контроль. Нужен либо снимок по НОМЕРУ КАДРА
вместо времени, либо уровень без океана/surf-sim. Это блокирует не только S15 — на этот контроль
опираются все визуальные критерии дальше.

**Чего в S15 сознательно нет:** снапа к текселям (план это разрешает: «стабильность даёт разрешение
+ EVSM»), так что ползание кромок при движении камеры БОЛЬШЕ legacy — цифру ползания надо снять
серией облёта до S16, как план и просил; EVSM (S16); динамического числа партиций (S17); HZB-кулла
в режиме; CPU-хвоста кастеров (их 0 на обоих уровнях, ридаут кричит жёлтым если станет больше).

### S16 — EVSM4 + блюр СДЕЛАНЫ; проверка глазами вскрыла ЛОЖНЫЙ КОНТРОЛЬ во всей сессии (2026-09-14)

**S16a (моменты) и S16b (блюр) — готовы.** `sdsm_evsm_cs.hlsl` конвертирует depth-атлас в
моменты (pos, neg, pos², neg²) одним диспатчем на весь атлас — партиция выводится из тайла, а не
из пермутации; `sdsm_evsm_blur_cs.hlsl` — сепарабельный бокс по `ComputeBlurData` сэмпла, с одной
нашей поправкой: **тапы клампятся к ТАЙЛУ**, потому что у сэмпла партиции лежат слоями массива и
уйти в соседнюю нельзя физически, а у нас они в одном атласе и ядро в 16 текселей перешагивает
4-текселевый жёлоб в чужую партицию. Выборка — `CsmSampleEvsm`: один билинейный тап + две границы
Чебышёва, вся машинерия S8 (рампа, 6×6 gather, sharpen) обойдена намеренно.

Цена (wind_test, атлас 2048, два прогона на конфигурацию):

| | `Pass_SdsmMoments` | `Pass_Lighting` | `GPU.Frame` |
|---|---|---|---|
| SDSM + PCF | — | 0.107 | 2.918 / 2.849 |
| SDSM + EVSM без блюра | 0.14 | ~0.105 | — |
| SDSM + EVSM + блюр | 0.320 / 0.312 | 0.104 / 0.109 | 3.155 / 3.156 |
| legacy PCF @4096 | — | 0.077 / 0.079 | 2.933 / 2.905 |

Главное в этой таблице — **`Pass_Lighting` не изменился**: граница Чебышёва стоит не дороже 6×6
gather-PCF, который она заменяет. Вся цена EVSM — в пассе моментов (0.14 мс) и блюре (+0.18 мс).

Дефолт блюра пришлось менять **0.02 → 0.002**. У сэмпла `edgeSofteningAmount` — доля партиции, и
0.02 у них разумны, потому что их партиции нормализованы; у нас партиция p3 — 164 метра, и 0.02 от
неё это пенумбра 3.3 метра, которая размазывает сцену в плоское серое. Доля партиции как единица
оставлена (это правильная единица: мягкость — авторский МИРОВОЙ размер, а не число текселей,
меняющее смысл между партициями), сменено только число.

**Две находки GBV и правило, которое из них следует.** id=1358: блюр биндил атлас моментов как SRV,
пока его layout — `UNORDERED_ACCESS`; поэтому в блюре ОБЕ стороны UAV (альтернатива — гонять layout
между каждой парой диспатчей, четыре лишних барьера ради одной типизированной загрузки). id=1334,
затем id=1350 «does not support barrier interop»: текстуры создавались сырым
`CreateCommittedResource`. **Под enhanced barriers у текстуры есть LAYOUT, и создавать её можно
только через `render::CreateCommittedTexture` (`rendering/core/TextureCreate.h`) —
`CreateCommittedResource3` с барьерным layout'ом.** Это единственное место в движке, где texture
создаётся, и обход его валиден ровно до первого барьера.

#### S16.1 — контроль ВРАЛ: `--shadow-mode=csm` всю сессию запускал VSM (2026-09-14)

Сначала этот раздел объявлял дефект: при равной плотности текселей SDSM якобы мылит вчетверо
сильнее legacy. **Дефекта нет. Неверен был контроль**, и вот одна строка, `sources/app/main.cpp:526`:

```cpp
render::g_shadowMode = (strncmp(p, "legacy", 6) == 0) ? Legacy
                     : (strncmp(p, "sdsm",  4) == 0)  ? Sdsm
                                                      : VSM;   // ВСЁ ОСТАЛЬНОЕ
```

Все «legacy»-прогоны шли с `--shadow-mode=csm` — очевидное написание для режима, который везде ещё зовётся CSM
(ручки `csm.*`, пасс `Pass_CSM`, флаг `--csm-readout`, сам атлас). Токен не узнан → тихий фолбэк на **VSM**.

Почему это не всплыло сразу: в том плече молча инертно ВСЁ, что начинается на `csm.` — атлас, фильтр,
bias, тинт (`csmDebugMode = vsmDir ? 0u : ...`, `SceneRenderer_Lighting.cpp:378`). Каждый мой «изолирующий»
прогон на legacy давал 0.1–1.9 % изменённых пикселей и читался как «эта ручка ни при чём», а на самом деле
значил «ручка вообще не в этом режиме». Сломалось на атласе **512**: legacy с текселем 94 мм рисовал
идеальную гребёнку листиков — физически невозможно. Добил сдвиг выборки на 100 текселей (`csm.normalBias:100`):
SDSM развалился на **83.8 %** пикселей, «legacy» — на **0.1 %**. Он не читал атлас вообще.

**Что показывает ПРАВИЛЬНЫЙ контроль** (`--shadow-mode=legacy`). Мера — НЕ «разница с legacy» (она растёт
от любого сдвига границ партиций и ничего не говорит о качестве), а АБСОЛЮТНАЯ детальность тени:
среднеквадратичная энергия лапласиана по кропу 480×310 чистого песка (без стволов и листвы).

| атлас | legacy CSM | SDSM | VSM |
|---|---|---|---|
| 512 | 9.744 | — | 11.75 |
| 2048 | **9.631** | **9.613** | 11.74 |
| 4096 | 9.910 | **10.550** | 11.74 |
| 8192 | 11.870 | **14.917** | 11.74 |

И глазами то же: на 2048 картинки практически неразличимы, на 4096 SDSM заметно чётче на гребёнке листиков.

Цена при равном атласе 2048 (по два прогона):

| | теневая цепочка | `Pass_Lighting` | `GPU.Frame` |
|---|---|---|---|
| legacy @2048 | `Pass_CSM` 0.228 / 0.230 | 0.076 / 0.085 | 2.798 / 2.864 |
| legacy @4096 | `Pass_CSM` 0.301 / 0.298 | 0.078 / 0.079 | 2.930 / 2.928 |
| SDSM @2048 PCF | shadow 0.371/0.339 + analyze 0.043/0.039 | 0.107 / 0.108 | 2.918 / 2.849 |
| SDSM @2048 EVSM+блюр | + moments 0.320 / 0.312 | 0.104 / 0.109 | 3.155 / 3.156 |

**Честный вывод: на равном атласе SDSM даёт ТУ ЖЕ резкость, что legacy, за +0.05 мс кадра и
+0.17 мс на теневой цепочке.** Выигрывать он начинает с 4096 и сильно выигрывает на 8192 — то есть там,
где legacy уже упирается в свою сферу, а SDSM всё ещё масштабируется. Тексельный ридаут при этом НЕ врал:
SDSM p1 21.16 мм против legacy c0 22.675 мм при 2048 — ровно паритет, который и виден на картинке.

Ограничение метрики, чтобы не переоценить: лапласиан считает СТУПЕНЬКИ за деталь, поэтому очень
низкое разрешение даёт ложно-высокий балл (отсюда legacy@512 = 9.744 выше, чем @2048). В диапазоне
2048–8192 она согласуется с глазами, ниже — нет.

**Починено, чтобы не повторилось** (это ВТОРОЙ раз, когда `--shadow-mode` врёт: в первый его молча
перетирал `graphics_settings.json`, см. комментарий над `g_shadowModeFromCli`):

* `legacy` И `csm` — оба дают Legacy CSM; `sdsm`; `vsm`;
* неузнанный токен больше НЕ переключает режим и не считается override'ом — решает файл настроек;
* `Scene::ReconcileShadowMode` пишет **АКТИВНЫЙ режим в лог при каждой его смене** (раньше legacy не объявлял
  себя никак, и режим приходилось угадывать по побочным строкам вроде `[VSM] allocated`) плюс WARN
  на проигнорированный флаг. Проверено тремя прогонами: `legacy` и `csm` → `directional shadow mode:
  Legacy CSM (--shadow-mode)`, `nonsense` → WARN + режим из настроек.

**Урок (третий раз за шаг):** прежде чем строить A/B, НАДО УБЕДИТЬСЯ, ЧТО КОНТРОЛЬНОЕ ПЛЕЧО
ДЕЙСТВИТЕЛЬНО ТО, ЧЕМ НАЗЫВАЕТСЯ. Дешёвый тест на это — ПОЛОМАТЬ его нарочно
(здесь: атлас 512 и сдвиг выборки на 100 текселей): плечо, которое НЕ реагирует на разрушение
собственной фичи, эту фичу не использует.

#### S16.2 — дальняя граница больше не прибита к параметру (2026-09-14, СДЕЛАНО)

Вопрос юзера, и он по делу. Было (`shaders/sdsm_analyze_cs.hlsl:381`):

```hlsl
intervalBegin = (i == 0)       ? gNearZ : SdsmLogPartitionFromRange(i,   minZ, maxZ, count);
intervalEnd   = (i == count-1) ? gFarZ  : SdsmLogPartitionFromRange(i+1, minZ, maxZ, count);
```

ВНУТРЕННИЕ границы ездили по редуцированному `[minZ, maxZ]`, а крайние были прибиты к near камеры и к
`gFarZ` (= `cascadeConfig_.maxDistance`, дальность теней из настроек). Теперь оба конца едут. Ридаут
до/после, та же камера:

```
было:   p0 interval 0.01-3.18   ...   p3 interval 35.85-1000.00  samples 50716
стало:  p0 interval 0.95-3.18   ...   p3 interval 35.85-120.43   samples 50715
```

`maxZ = 120.43` — ровно то, что восстанавливалось из геометрической прогрессии границ (≈ 120 м).

**Почему правка ПАРНАЯ, и почему вторая половина оказалась проверкой, а не кодом.** Сжать последний
интервал нельзя, если приёмник за `maxZ` перестанет попадать в партицию вообще. Оказалось, кламп
уже есть ПО ПОСТРОЕНИЮ: `CsmChooseCascade` считает, сколько ВНУТРЕННИХ границ (`splitsVS.yzw`)
пройдено, и возвращает 0..3 — то есть всё, что дальше последней границы, и так попадает в последнюю
партицию. Сам `farSplit` доходит только до `sFar` полосы смешивания, которую гард `idx < 3` для
последней партиции не использует. А бокс не сдвинулся потому, что сэмплы, задающие его, — те же
самые, что задают `maxZ` (163.0 вместо 162.0 м — шум ветра, samples 50715 вместо 50716).

**Кто реально читал прибитую границу** и ради чего правка сделана: `Finalize` строит из интервала
**ОБЪЁМ КУЛЛА** (`SdsmCullVolumePlanes(sliceNear, intervalEnd, ...)`) и потолок/fallback по сфере слайса.
При `35.85..1000` кастеры субмитились по восьмикратно более длинной призме, чем занято геометрией, а
fallback-бокс пустой партиции был сферой всей этой призмы. Для p0 то же вниз (0.01 вместо 0.95) плюс
ширина полосы смешивания первой партиции, которая теперь её настоящий слайс.

**ВЫИГРЫША ПО ВРЕМЕНИ НЕТ, и это надо сказать прямо.** Четыре прогона после против двух до:
`Pass_SdsmShadow` 0.366–0.374 против 0.339–0.371, `GPU.Frame` 2.893–3.060 против 2.849–2.918 — диапазоны
пересекаются, разница в шуме. Причина понятная: на wind_test за 120 м КАСТЕРОВ НЕТ, так что сжатие
пустой призмы ничего не экономит. Правка про КОРРЕКТНОСТЬ, а не про скорость; платить она будет
там, где геометрия сильно ближе дальности теней (помещение при `maxDistance` 300).

Картинка: 0.1 % пикселей, HF-rms 9.613 → 9.647. Дальний план теней не потерял.

#### S16.3 — выключение EVSM убивало устройство (2026-09-14, ПОЧИНЕНО)

Юзер ловил в Release_Editor на снятии галки EVSM: `D3D12: Removing Device` и каскад `_com_error`,
следом «acceleration-structure or bindless-table allocation failed» — это уже последствие, а не причина.

Причина одна строка в `Scene::UpdateSdsmFrameParams`:

```cpp
const bool evsmReady = wantEvsm ? sdsm_.EnsureMoments(renderer, D.shadowRes)
                                : (sdsm_.ReleaseMoments(renderer), false);
```

`ReleaseMoments` делает немедленный `Reset()`, а вызывался посреди кадра — то есть атлас моментов
(64 МБ на 2048) уничтожался, пока кадры в полёте его ещё читают.

Починено переносом резидентности моментов в `Scene::ReconcileShadowMode` — туда, где уже живут пул VSM,
буферы SDSM и реаллокация атласа, и где уже есть `WaitForPreviousFrame()`. Пер-кадровый путь теперь
только СПРАШИВАЕТ (`MomentsReady() && MomentsRes() == D.shadowRes`) — второе сравнение гасит тот
единственный кадр, где deferred-размер атласа и согласованный могут разойтись: EVSM просто
пропускает такой кадр на PCF-ветке. Заодно `ReleaseMoments` теперь логирует освобождение: «атлас
появился» без парного «исчез» неотличим в логе от течки на 64 МБ.

Проверка — `--sweep=sdsm.evsm:1,0,1,0,1,0` в Release_Editor: шесть переключений, шесть пар
allocated/released в логе, ноль device removal, exit 0. **`--sweep` и есть способ воспроизводить
рантайм-тоггл без рук** — он применяет значение перед каждым шотом тем же диспетчером, что и `--set`.

**Правило, которое из этого следует:** ресурс, который может быть ОСВОБОЖДЁН по рантайм-ручке,
освобождается ТОЛЬКО в резидентностной согласующей точке с GPU-idle. Пер-кадровый код вправе
только СПРОСИТЬ, есть ли ресурс. Контракт записан над `EnsureMoments`/`ReleaseMoments`.

Гейты после S16.2+S16.3, все в режиме SDSM с EVSM: Debug `--gbv --barrier-cmp` — ноль ошибок
валидации (в `gbv.log` только id=274, живые объекты на выходе), кадр снят; Release
`--scene-stress=4 --barrier-cmp` — `verdict: CLEAN`, `barriers: emit enhanced=2768 legacy=0`, нового
`logs/barrier_diag.log` нет; `cull validation PASS: 46 views + camera match CPU`; три конфига
собираются, `check_shaders` 97/97, `check_logging` 0.

#### S16.4 — ПРИЁМКА по спеке: что закрыто, что нет (2026-09-14)

Шаг СДЕЛАН НАПОЛОВИНУ. В спеке S16 четыре компонента — EVSM4, блюр, мипы, shadow-MSAA —
сделаны первые два.

| компонент | статус |
|---|---|
| EVSM4 (варп, моменты, Чебышёв) | СДЕЛАНО |
| сепарабельный бокс-блюр | СДЕЛАНО |
| мипы + `SampleGrad` (S16c) | **НЕТ** — лайтинг в compute, нужны АНАЛИТИЧЕСКИЕ производные |
| shadow-MSAA + `sdsm.shadowAA` (S16d) | **НЕТ** — нужен MSAA-scratch и перестройка depth-пасса |

По критериям приёмки:

| критерий | вердикт |
|---|---|
| кромки без ступенек, нет шума тапов | ЗАКРЫТО на атласе 2048; НА 1024 НЕТ (см. ниже) |
| роща — протечек нет, тень не отрывается от ствола | ЗАКРЫТО — PCF и EVSM у основания ствола неразличимы |
| дальние каскады без муара | **НЕ ПРОВЕРЕНО** — муар без мипов живёт в ДВИЖЕНИИ, статикой не ловится |
| облёт — ползание ниже цифры S15 | **НЕ СДЕЛАНО** — и самой цифры S15 тоже нет |
| **ставка: 1K-тайлы против legacy 2K-PCF** | **ВЗЯТА на PCF** (9.61 против 9.91 при 8.4 МБ против 33.6); EVSM её ломает |
| память и время в ридауте/профдампе | ЗАКРЫТО |
| VSM и legacy паритет | ЗАКРЫТО — оба в пределах шумового пола |
| полный набор гейтов | ЗАКРЫТО |

**Паритет соседних режимов** измерен против ШУМОВОГО ПОЛА, а не против нуля: два прогона ОДНОЙ
конфигурации на ПОЛНОМ кадре дают 3.01 % различающихся пикселей (океан, SSR, DLSS). Legacy до/после
правок S16.2/S16.3 — 2.44 % и 3.03 %, VSM — 2.20 %. Всё в поле.

**СТАВКА ВЗЯТА — НА PCF. EVSM её ЛОМАЕТ, и это два РАЗНЫХ вывода.** Спека требовала
«если бьют по глазам — память ниже нынешней». Сначала здесь было написано «по картинке да, по
памяти нет» — потому что в таблице стоял только EVSM-вариант. Без EVSM картина другая:

| | картинка (HF) | `GPU.Frame` | память |
|---|---|---|---|
| legacy @2048 PCF | 9.63 | 2.83 | 8.4 МБ |
| legacy @4096 PCF | 9.91 | 2.93 | **33.6 МБ** |
| **SDSM @2048 PCF** | **9.61** | 2.88 | **8.4 МБ** + 17 КБ |
| SDSM @2048 EVSM+блюр | ~10.1 | 3.18 | **92.3 МБ** |
| SDSM @1024 EVSM+блюр | **СТУПЕНЬКИ** | 2.95 | 23.3 МБ |

**Legacy упирается в свою сферу, а не в атлас**: от 2048 к 4096 его картинка растёт 9.63 → 9.91,
то есть вчетверо больше памяти покупают 3 %. SDSM на 2048 с тем же PCF даёт 9.61 — ту же
картинку, что legacy на 4096, при **вчетверо меньшей памяти** (8.4 против 33.6 МБ) и +0.05 мс
кадра. Оба условия спеки выполнены одновременно.

**Без EVSM режим почти ничего не стоит по памяти сверх самого атласа**, который он делит
с legacy: управляющие буферы — **~17 КБ** (партиции 640 Б, их копия за прошлый кадр, z-bounds,
аккумулятор боксов, буфер фрустумов на 47 видов = 12 КБ — самый крупный кусок, четыре
per-view CB, cull-CB и кольцо ридбэка).

**А вот EVSM — отдельный размен, и он НЕ ОКУПИЛСЯ.** Он добавляет 84 МБ (67.1 моменты
RGBA32F + 16.8 scratch блюра) и +0.30 мс кадра ради мягкости, которая на этой сцене в
пределах различимого (9.61 → ~10.1). То есть одиннадцатикратная память за проценты картинки.
Владелец посмотрел и сказал: «с текущей фильтрацией тени смысла в нём не вижу» — и цифры с
этим согласны. Дефолт `sdsm.evsm` остаётся **OFF**; код сохранён как A/B и как база для S17/S18.

Если к EVSM возвращаться, то первым делом формат: только **EVSM2 (RG32F)** режет вдвое без
потери точности; **fp16 отпадает по арифметике** (S16.6).

**Атлас 1024 с EVSM — НЕ РАБОЧАЯ ТОЧКА, и метрика это СКРЫЛА.** HF-rms на одном кропе:
legacy@4096 = 11.32, EVSM@1024 = 11.44, EVSM@2048 = 11.54 — по числам все трое ровня. Глазами
EVSM@1024 — грубая лесенка по всем кромкам. Лапласиан считает ступеньку за деталь — ровно та
оговорка из S16.1, теперь с конкретным примером. Причина физическая: **PCF-ядро 6×6 текселей шире,
чем блюр моментов в 2 текселя**, то есть на низком разрешении PCF сам себя сглаживает, а EVSM без
мипов — нет. Это же аргумент за S16c: мипы нужны не только против муара, но и против ступенек.

**Цена пасса моментов, разобрана (атлас 2048).** Блюр не запускается вовсе при ширине ≤ 1 текселя
(`SdsmShadows.cpp:512`), поэтому `sdsm.blur:0.0001` и `sdsm.blur:0` дают одно и то же:

| | `Pass_SdsmMoments` |
|---|---|
| без блюра (одна конверсия) | 0.141 |
| блюр 2 текселя (дефолт) | 0.322 |
| блюр 16 текселей (кап) | 0.403 |

То есть САМ ФАКТ запуска блюра стоит +0.181 мс, а расширение ядра восьмикратно — всего +0.081.
Пасс упирается в ПАМЯТЬ, не в тапы. И там есть ОБРЫВ по размеру атласа:

| атлас | моменты, мс | трафик/кадр | эфф. ГБ/с |
|---|---|---|---|
| 1024 | 0.274 | 86 МБ | 314 |
| 2048 | 0.322 | 344 МБ | 1068 |
| 4096 | **2.035** | 1376 МБ | 676 |

1024 → 2048: трафик вчетверо, время в 1.18 раза — на 1024 пасс упирается в накладные девяти
диспатчей, не в память. На 2048 эффективные 1068 ГБ/с выше любой DRAM — тайл блюра (16.8 МБ)
сидит в L2. **На 4096 тайл становится 67 МБ, из кэша выпадает, и время прыгает в 6.3 раза.**
Рабочая точка EVSM — ровно 2048: ниже ступеньки, выше обрыв.

Мелочь рядом: `widthTexels` считается от `tile` (1024), а бокс партиции мапится на `contentRes`
(1016) — комментарий рядом обещает второе. Расхождение 0.8 %, не починено.

И ещё один открытый пункт S15 ПОПУТНО ЗАКРЫЛСЯ: детерминированный пиксельный контроль есть —
это **`--wind-freeze=0`**, который пинит ветровой час и даёт ПОБАЙТОВО идентичную геометрию между
прогонами. Весь S16 измерен именно так. Остаточный шум — не ветер, а океан/SSR/DLSS,
и на кропе суши он 0.1 %, на полном кадре 3 %.

#### S16.5 — решение по S16c и S16d (2026-09-14)

**S16d (shadow-MSAA, `Texture2DMS` 4× + ручка `sdsm.shadowAA`) — ОТМЕНЁН владельцем.**
Ручки `sdsm.shadowAA` в коде нет и не будет — удалять нечего, только вычеркнуть из спеки ниже, чтобы
не числиться обещанным контролом.

**S16c (мипы + `SampleGrad`) — ИЗМЕРЕНО, что он даст, и рекомендация — НЕ ДЕЛАТЬ пока.**

Минификация (тексель тени мельче экранного пикселя) у нас бывает не на фронтальных
поверхностях (там везде магнификация: p0 6.1 мм против пикселя 4.2 мм на 3 м, p3 160 мм против
166 мм на 120 м), а на СКОЛЬЗЯЩИХ — на пляже, куда камера смотрит почти горизонтально.
Замер именно там, дальний песок, **`--dlss=off`** (иначе темпоралка съедает ровно то, что меряем):

| | HF-rms | сильных кромок |
|---|---|---|
| PCF 6×6 | 25.82 | 41.98 % |
| EVSM + блюр | **29.92** | **44.95 %** |

Глазами то же: у PCF кромки теней гладкие, у EVSM — зубчатые. **То есть да, EVSM без мипов
алиасит на минификации сильнее, чем PCF, и S16c это лечит.** Причина та же, что и со
ступеньками на 1024: ядро PCF 6×6 шире, чем блюр моментов в 2 текселя.

**НО:** с ВКЛЮЧЁННЫМ DLSS (боевой режим) та же пара неразличима — темпоралка гасит именно
этот класс алиасинга. А цена S16c падает ровно в самое слабое место EVSM:

* мип-цепочка моментов = **+33 % к атласу** (92 → ~114 МБ на 2048), а память уже втрое хуже legacy
  и именно она не даёт закрыть ставку S16;
* генерация мипов каждый кадр — ещё трафик в пасс, который УЖЕ bandwidth-bound (S16.4);
* нужны АНАЛИТИЧЕСКИЕ производные UV — лайтинг в compute, ddx/ddy нет.

**Вывод: S16c улучшает то, что и так гасит DLSS, а платит той самой памятью и полосой, из-за
которых шаг и не закрывается.** Правильный порядок — сначала формат (RGBA16F или EVSM2),
и только потом, если появится сценарий без DLSS, возвращаться к мипам.

#### S16.6 — формат моментов: fp16 ОТПАДАЕТ ПО АРИФМЕТИКЕ, путь — EVSM2 (2026-09-14)

Память — единственное, что не даёт закрыть ставку S16, и очевидный ход — урезать 16 байт на тексель
вдвое. Два способа, и они НЕ равноценны.

**Владелец заподозрил, что с fp16 полезет акне. Подозрение верное, механизм другой.**
Граница Чебышёва считает `var = E[x²] − E[x]²`, а это КАТАСТРОФИЧЕСКАЯ ОТМЕНА: на плоском
приёмнике оба члена равны, и результат — чистая ошибка округления. Сравниваем её с `minVariance`,
тем самым порогом, который шейдер уже применяет:

| | шум дисперсии / minVariance |
|---|---|
| EVSM4 сейчас, fp32, c=42 | **0.0096** (в 100 раз НИЖЕ порога) |
| fp32, c=5.5 | 0.53 |
| **fp16, c=5.5** | **6 470** |
| fp16, c=4.0 | 8 860 |
| fp16, c=3.0 | **15 000** |

То есть в fp16 дисперсия превращается в шум на три-четыре порядка выше порога. И самое важное:
**опускать экспоненту, чтобы влезть в fp16, ДЕЛАЕТ ХУЖЕ**, а не лучше: `minVariance ∝ c²`
падает быстрее, чем ошибка. Проявляется это в основном как светотечка и дрожащая полутень
(завышенная var → Чебышёв → ±«освещено»), а пятнами, где var уходит в минус и клампится, — как
то самое акне. Расчёт воспроизводим на numpy за десяток строк, пересчитать проще, чем спорить.

**Поэтому путь — EVSM2 (RG32F): та же экономия вдвое, без потери точности.**

| | моменты + scratch @2048 |
|---|---|
| RGBA32F EVSM4 (сейчас) | 83.9 МБ |
| **RG32F EVSM2** | **41.9 МБ** |
| RGBA16F EVSM4 | 41.9 МБ, но арифметика сломана |
| legacy D16 @4096 | 33.6 МБ |

С depth-атласом это 50 МБ против 34 у legacy — в полтора раза, а не втрое. И трафик пасса
моментов тоже вдвое — то есть заодно отодвигается L2-обрыв из S16.4 (тайл блюра на 4096
станет 33 МБ вместо 67).

**Что теряем с EVSM2 — измерено.** Отрицательный варп ловит остаток протечки. На wind_test
его вклад снят свипом `sdsm.evsmNeg` от 1 до 42 при шумовом поле 3.01 % на полном кадре:

* кроп суши (тени на песке): **0.08 %** — не делает ничего;
* полный кадр: 4.90 %, и карта разницы (усилена ×12) показывает тонкие штрихи СТРОГО по кромкам
  самозатенения листвы (верхняя треть 7–10 %, нижняя 0–1.3 %); песок и небо — чёрные.

То есть отрицательный варп работает только там, где лист затеняет лист вплотную, и не влияет на тени
на земле вообще. ОГОВОРКА честности: `evsmNeg:1` — это СЛАБЫЙ отрицательный варп, а не
его отсутствие; точный вердикт по EVSM2 требует реализации и взгляда на плотную рощу.

#### S16.7 — дрожание границ партиций: причина и ручка `sdsm.stabilityFrames` (2026-09-14)

Юзер: «из-за движения пальм на определённых ракурсах дрожит граница каскадов», потом — «даже
при замороженном ветре». Вторая реплика и оказалась ключом.

**Инструменты, которые это раскопали** (оба полезны дальше):
* серия `--wind-freeze=0 --shot-count=8 --shot-step=0` + `--csm-tint`, пиксели классифицируются по
  цвету тинта в ДОЛЮ ЭКРАНА на партицию — даёт число там, где было «дрожит/не дрожит»;
* `--sdsm-readout` теперь печатает **ВОСЕМЬ КАДРОВ подряд**, а не один: одна таблица говорит,
  что партиции ЕСТЬ, а вопрос был про то, что они ДЕЛАЮТ между кадрами;
* `--cam-pos` / `--cam-rot` из HUD — ракурс юзера воспроизводится дословно. На МОЕЙ камере
  (низко, среди пальм) дрожания НЕ БЫЛО вообще — без его ракурса баг был бы невоспроизводим.

**ПРИЧИНА, покадровый ридаут на его ракурсе со СТАТИЧНОЙ сценой:**

```
maxZ:  154.26 -> 166.05 -> 153.76 -> 154.30   (12 метров)
minZ:   15.23 ->  15.24 ->  15.23 ->  15.23   (стоит)
```

Дрожит **не minZ от пальм, а maxZ от ОКЕАНСКИХ ВОЛН**: гребни открывают и закрывают самую
дальнюю видимую точку. Все внутренние границы логарифмичны по `[minZ, maxZ]`, так что 8 %
на дальнем конце двигают их все сразу.

**ПЕРВАЯ ВЕРСИЯ ФИКСА БЫЛА НЕВЕРНОЙ, и это стоит запомнить.** Сглаживался сам диапазон,
с асимметрией «расширение мгновенное, сжатие ленивое» — чтобы сглаженный диапазон оставался
надмножеством измеренного и кулл гарантированно ничего не терял. Безопасно — и бесполезно:
колебание ЭТО И ЕСТЬ расширение через кадр, и мгновенная сторона пропускала его целиком
(замер: 0.70 % площади экрана за кадр против 0.64 % сырого — никак).

**Рабочая версия: КОНЦЫ ДИАПАЗОНА ОСТАЮТСЯ СЫРЫМИ, сглаживаются только ВНУТРЕННИЕ
границы.** Покрытие и кулл теперь НЕ ЗАВИСЯТ от сглаживания вообще: объём кулла и потолок
по сфере строятся из СОБСТВЕННЫХ концов партиции, а `intervalBegin[0]` и `intervalEnd[last]` —
ровно то, что измерила редукция. Контигуальность цела: граница i считается один раз и служит и
`intervalEnd[i-1]`, и `intervalBegin[i]`. И главное — освободившись от обязанности быть
надмножеством, среднее стало СИММЕТРИЧНЫМ — а только оно и гасит двустороннее колебание.
Состояние едет в запасной паре партиции 0 (`smoothedRange`), а не выводится из концов —
они теперь сырые и авераж продолжить не могут; Finalize больше не затирает это поле.

| `sdsm.stabilityFrames` | скачок границы за кадр |
|---|---|
| 0 (выкл) | 0.540 % |
| 4 | 0.076 % |
| **30 (дефолт)** | **0.049 %** |
...
**ЕДИНИЦЫ — КАДРЫ ПОЛУЗАТУХАНИЯ, не доля.** Доля (0.977, 0.9971 …) — то, что нужно шейдеру,
и как настройка она нечитаема: всё полезное прячется между 0.97 и 0.999, что линейный
слайдер 0..1 не разрешает вообще. Хранится, типится и сохраняется в КАДРАХ; конверсия
в долю — в ОДНОМ месте, при заполнении CB. Ключ json тоже переименован в `stabilityFrames`,
иначе старое сохранённое `0.977` прочлось бы как «0.977 кадра», то есть как выключено.
Кадры, а не секунды, потому что усреднение и есть покадровое: ветровой час можно
заморозить, и затухание по времени при dt = 0 перестаёт сходиться.

Цена инертности — лаг: границы догоняют сцену, так что на резком повороте или склейке
граница может мгновение постоять не там. Записано в хинте ручки.

Побочно измерено и важно для будущих разборов «что-то дрожит»: на ПОЛНОСТЬЮ статичной
сцене покадровые изменения составляют 3.71 % полного кадра с DLSS и 0.53 % без него,
причём в зоне теней на песке 0.16 % против 0.00 %. То есть кипение на кромках листвы —
темпоральное от DLSS и к SDSM отношения не имеет.

#### Резидентность моментов: полностью динамическая

Проверено логом: атлас моментов резидентен ТОЛЬКО когда режим = SDSM И `sdsm.evsm` = 1.
Прогон `--shadow-mode=legacy --set=sdsm.evsm:1` не выделяет ни моментов, ни буферов SDSM
вообще (строки про атлас нет). Снятие галки освобождает 64 МБ + 16 МБ scratch в тот же
реконсиль (S16.3), и обе стороны пишутся в лог.
...
**Что реально изменилось в коде (S15, 2026-09-14).** Контракт партиции — пара зеркал
`sources/rendering/shadows/SdsmPartitions.h` ↔ `shaders/sdsm_partitions.hlsli` (static_assert на
144 байта). Анализ — `shaders/sdsm_analyze_cs.hlsl`, шесть точек входа под одной root-сигнатурой
(`ClearBounds`, `ReduceZBounds`, `LogPartitions`, `ReduceBounds`, `Finalize`, `CarryFrustums`),
владелец `SdsmShadows` (буферы, PSO, ридбэк-ринг). Режим — `render::ShadowMode::Sdsm` + `SdsmActive()`,
`--shadow-mode=sdsm`, `"mode": "sdsm"`, Ctrl+V теперь ЦИКЛ из трёх, в dev-окне вместо двух чекбоксов
один комбо (чекбокс «Legacy активен» при третьем режиме врал бы).

Четыре отступления от плана, найденные по дороге:
* **ни одной пермутации шейдера вне собственного анализа.** План предполагал вариант шейдера
  глубины, читающий матрицу из буфера. Вместо этого анализ пишет ГОТОВЫЙ константный блок, а CPU
  биндит его адрес (S15.2 выше). Тем же приёмом получен и HZB без правки кулла;
* **кулл не понадобилось параметризовать.** План хотел вариант `shadow_cull_cs` «планы из буфера».
  Оказалось, шейдер уже читает планы из structured buffer, а его цикл идёт по `[0, gNumViews)` —
  то есть меньший `gNumViews` И ЕСТЬ поддиапазон. `Main_SdsmCull` — это тот же clear+cull с
  `gNumViews = 4` и SDSM-буфером на t1; ни байта шейдера не тронуто, legacy/VSM байт-в-байт по
  построению. Основной кулл остаётся в голове кадра (его камерная строка кормит indirect G-buffer),
  а строки 0..3 он грузит reject-all'ом, раз их всё равно перезапишут;
* **флип float обязателен.** Сэмпл редуцирует light-ТЕКСКООРДЫ в [0,1], где `asuint` монотонен;
  у нас light-space МЕТРЫ, они регулярно отрицательны, и сырой `InterlockedMin(asuint(x))` там
  упорядочивает задом наперёд. `SdsmFloatFlip` — стандартный order-preserving флип radix-сортировки;
* **потолок вместо `mMaxScale`.** Сэмпловая магическая константа заменена на бокс сферы S1 того же
  интервала (`SdsmSliceSphereRadius` — транскрипция `ComputeCascadeSphere` в HLSL). Он же служит
  фолбэком для пустой партиции и ПОЛОМ (бокс никогда не шире сферы: шире — значит редукция принесла
  мусор, а не геометрию).

**Две ошибки, которые поймал GBV, и обе Release бы проглотил молча:**
* `StructuredBuffer<float4>` над `ShadowGpuData.ViewFrustums`, чей SRV создан со stride 256 →
  `GPU-BASED VALIDATION id=1387: byte stride mismatch` и доступ в 16 раз мимо. Чтение идёт через
  `SdsmViewFrustum` (16 планов, 256 байт);
* буфер планов был размером `kMaxShadowViews`, а `ViewFrustumCount()` возвращает **47**, не 46 —
  у ShadowGpuData за теневыми видами лежит слот КАМЕРЫ (occlusion S4). `CarryFrustums` писала на
  один вид за конец. Буфер теперь `kMaxShadowViews + 1`, и копия клампится по обеим сторонам.
...
отрицательный ловит остаток; ручки `sdsm.evsmPos/Neg`, `sdsm.blur`. (~~`sdsm.shadowAA`~~ — shadow-MSAA ОТМЕНЁН 2026-09-14, см. S16.5; ручки нет в коде.)
