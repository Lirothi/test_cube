...
> ⚠ **Читать сразу «S16.1» ниже.** Весь первый заход сравнивал SDSM не с legacy, а с **VSM**:
> `--shadow-mode=csm` не узнавался и тихо фолбэчил на VSM. Правильный контроль: на равном атласе
> у SDSM и legacy ПАРИТЕТ по резкости, с 4096 SDSM впереди. Цифры S15.5 ниже сняты тем же багом.
...
| **ставка: 1K-тайлы против legacy 2K-PCF** | ЧАСТИЧНО — картинка да, **память НЕТ** |
...
**Ставка, честно.** Спека требовала «если бьют по глазам — память ниже нынешней»:

| | картинка | `GPU.Frame` | память |
|---|---|---|---|
| legacy @4096 PCF | база | 2.93 | **33.5 МБ** |
| SDSM @2048 EVSM+блюр | сопоставимо/чуть чётче | 3.18 | **92 МБ** (67 моменты + 8.4 depth + 16.8 scratch) |
| SDSM @1024 EVSM+блюр | **СТУПЕНЬКИ** | 2.95 | 23 МБ |

По картинке ставка берётся, по памяти — нет: RGBA32F втрое дороже legacy-атласа, а не дешевле.
Чтобы ставка закрылась целиком, моменты должны уменьшиться вдвое — RGBA16F (экспонента ~5.5 в fp16
живёт: exp(5.5)² = 59874 < 65504) либо EVSM2 (RG32F, только положительный варп). Оба — размен на
большую светотечку, решение за владельцем.
...
#### S16.7 — временная стабилизация диапазона + где ДРОЖАНИЕ НА САМОМ ДЕЛЕ (2026-09-14)

Юзер: «из-за движения пальм на определённых ракурсах начинает дрожать граница каскадов». Диагноз
подтвердился ЧАСТИЧНО, и разница важна.

**Измеритель:** детерминированная серия `--wind-freeze=0 --shot-count=8 --shot-step=<sec>` с
`--csm-tint`, пиксели классифицируются по цвету тинта в долю экрана на партицию. Стоит запомнить: это
даёт число там, где раньше было только «дрожит/не дрожит».

**С ветром граница ДЕЙСТВИТЕЛЬНО ездит:** доля p0 ушла 7.7 % → 14.1 % экрана за 8 шагов,
до **3.3 % площади за ОДИН шаг**, при том что p2/p3 стоят (дельты 0.05–0.3 %). Механизм: границы
логарифмичны по `[minZ, maxZ]`, а `minZ` — это БЛИЖАЙШИЙ ВИДИМЫЙ ПИКСЕЛЬ, который качающийся
перед камерой лист двигает на метры — и тянет за собой ВСЕ границы сразу.

**Сделана ручка `sdsm.stability`** (дефолт 0.85, 0 = сырая редукция и A/B). Сглаживается
**ДИАПАЗОН, а не границы** (форма сплита остаётся целой), в ЛОГ-пространстве (сплит логарифмичен,
так что фиксированная доля там — фиксированная доля партиции на любой глубине), и главное —
**РАСШИРЕНИЕ МГНОВЕННОЕ, СЖАТИЕ ЛЕНИВОЕ**. Эта асимметрия делает правку безопасной по
ПОСТРОЕНИЮ: сглаженный диапазон ВСЕГДА надмножество измеренного, поэтому объём кулла из
него не может выкинуть кастера, которого сырой бы оставил, а партиция никогда не обязана
покрывать глубину, под которую не фитилась. Прошлый диапазон берётся из `PrevPartitions`
(после S16.2 `intervalBegin[0]` и есть minZ, `intervalEnd[last]` — maxZ), потребовался один из двух
резервных слотов CB, так что раскладка не сдвинулась. Счёт ПОКАДРОВЫЙ, а не по времени:
ветровой час можно заморозить, и dt-затухание при dt = 0 перестаёт сходиться вообще.

**А вот что замер СКАЗАЛ ПРОТИВ обеих гипотез.** Юзер добавил: «даже при замороженном
ветре вижу дрожачку тени». Серия с `--shot-step=0` (геометрия и камера СТАТИЧНЫ):

| | разброс доли p0 | покадровые изменения, полный кадр | они же на затенённом песке |
|---|---|---|---|
| DLSS вкл | **0.01 %** | 3.71 % | 0.16 % |
| DLSS выкл | **0.00 %** | 0.53 % | **0.00 %** |

**Граница партиций на статике НЕ ДРОЖИТ вообще** — то есть гипотеза «джиттер DLSS двигает
minZ через depth-буфер» НЕВЕРНА (хотя выглядела убедительно). А кипение на статике есть, и
оно **темпоральное от DLSS**: 3.71 % против 0.53 % без него, причём в зоне теней на песке
почти отсутствует (0.16 % против 0.00 %) — оно сидит на геометрических кромках листвы.

**ОГРАНИЧЕНИЕ ЗАМЕРА, честно:** `--shot-step` продвигает ветровой час между СНИМКАМИ, а
между ними проходят десятки реальных кадров, за которые покадровая EMA успевает сойтись.
Поэтому серия показывает ДРЕЙФ границы, но НЕ разрешает покадровое дрожание, ради которого
ручка и сделана. Что измеримо: путь ЖИВОЙ — stability 0 против 0.85 расходятся на 2.23 %
пикселей в среднем по серии. Вердикт по самому дрожанию — за глазом на живой камере.

Чтобы воспроизвести КОНКРЕТНЫЙ ракурс, где это видно: HUD печатает `Cam:` и `rot:`, а
`--cam-pos=x,y,z --cam-rot=x,y,z,w` восстанавливают его дословно — шот раунд-трипится в headless-репро.
