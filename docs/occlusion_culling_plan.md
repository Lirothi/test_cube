...
## S5b. Окклюжен со стороны СВЕТА: two-pass HZB для теневых вью (Legacy CSM по каскаду, VSM по странице) — S5b.1 (CSM) ✅ СДЕЛАН 2026-09-04 (uncommitted, **дефолт OFF по замеру**, ручка `csm.hzbCull`); S5b.2 (VSM) ⏳ не начат
...
### Откат
`csm.hzbCull:0` (дефолт) — три новых пасса не регистрируются, cull теней как сегодня; `vsm.hzbCull`
появится с S5b.2.
