...
## S6. Правила границ: тени, RT, редактор, гейты — ✅ СДЕЛАН 2026-09-05 (uncommitted)
...
### Что сделано (2026-09-05, как построено; плюс правило владения, которого в тексте выше нет)
1. **Владение (кто кулит indirect-объекты).** После S5 у объекта, который рисует GPU-driven
   G-buffer, было ДВА окклюдера: CPU-история (S3a/S3b, латентность 3 кадра) и two-pass HZB на GPU
   (S5, без латентности) — история только кормила бит вердикта, который GPU-тест и так перекрывает
   (у стены K=4: 540 одиночных + 228 групповых запросов в кадр впустую). Правило: пока S5 идёт
   (`GBufferIndirectThisFrame ∧ gbuffer.hzb`), `Scene::ApplyOcclusion` НЕ рассматривает
   `GBufferIndirect()`-объекты — вердикт сброшен, маска чанков остаётся фрустумной, решает GPU.
   CPU-путь (не взятые реестром объекты) и лайты (S3a.6) остаются у истории. Рычаг A/B
   `--set=vis.indirectQueries:1` (= до-S6 наложение), галка «also query GPU-driven objects» рядом с
   комбо Occlusion, счётчик `gpuOwned=` в строке `occlusion` readout'а и в `OcclusionFrameStats`.
2. **Тени не читают камеру — теперь это ассерт, а не комментарий.** `render::g_preparingShadowView`
   (thread_local: prepare теневого вью бежит на своей задаче параллельно камерной) выставляется
   RAII-гардом на время `PrepareViewQueue` теневого вью; `CameraOccluded()`,
   `ChunkCameraVisible()` и `ChunkCameraVisibleRef()` ассертят на нём в Debug. Единственный
   легальный читатель маски на каскадной стороне — счётчик S0 `AccumulateVisibility` — брал ссылку
   на маску и для каскадов (пользовался ею только для камеры); переписан: каскад маску не трогает.
   Дальше по коду: бит вердикта живёт в `CasterCam` и читается ТОЛЬКО камерной веткой cull'а
   (`gCamView`), теневые строки — фрустум (+ S5b). VSM не тронут.
3. **RT.** `RtSceneAs` собирает инстансы из `frame.objects` (все, кроме выключенных в редакторе),
   не из видимых бакетов — верно по построению; подтверждено логом `[RT] Acceleration structures:
   … 624 instances` при 172 объектах во фрустуме стены, и снимками ниже.
4. **Редактор.** Object ID у indirect-объектов: запись `InstancePerObject.objectId`
   (`ToObjectId32(GetEditorObjectId())`, смещение 192) → `gbuffer_indirect.hlsl` VS → PS-пермутация
   `EDITOR_OBJECT_ID` пишет RT4 → `Main_ObjectIdReadback` читает тот же RT, что у CPU-пути. Объект,
   скрытый окклюженом, в RT4 не попадает → клик по нему не выделяет ничего (корректно). Стенсил
   выделения (`GBuffer_Selected`) рисует выбранный объект своим CPU-draw'ом с depth-тестом —
   на indirect-путь не завязан. **Клик в Release_Editor руками не проверялся** (headless-пика нет).
5. **`--shadow-mode=`** — boot-override поверх `graphics_settings.json` (починено в S5b.2, см. там):
   без этого все гейты в «VSM» шли бы в Legacy.

### Замер (Release, ОДИН бинарь, `--dlss=off --wind-freeze --set=exposure.autoExposure:0`)
**Владение, стена K=4** (`--shadow-mode=legacy --set=scene.replicate:4`, `--profdump`, по два прогона):

| | queries ind/grp | gpuOwned | gpuHzbDeferred | GPU.Frame | Pass_OcclusionQueries GPU | CPU.Frame | Whole Cycle | prepareQueue | OcclusionQueries CPU |
|---|---|---|---|---|---|---|---|---|---|
| до S6 (`vis.indirectQueries:1`) | 540 / 228 | 0 | 8 | 2.781 / 2.768 | 0.060 / 0.055 | 2.785 / 2.778 | 2.712 / 2.699 | 1.354 / 1.336 | 0.448 / 0.458 |
| S6 (дефолт) | 1 / 0 | 3648 | 16385 | 2.737 / 2.715 | 0.008 / 0.007 | 2.743 / 2.720 | 2.629 / 2.580 | 1.160 / 1.129 | 0.039 / 0.040 |

GPU −0.05 мс, CPU-кадр −0.05, Whole Cycle −0.1 (prepareQueue −0.2, запись запросов −0.4 на
воркере); `GBuffer_Indirect` тот же (0.071–0.075: спрятанное теперь снимает GPU, а не CPU). Картинка
стены до/после владения: **0.005 %** (идентична).

**Границы** (снимки, % пикселей, пол 0.03 при однотапе): тени при `vis.method:0` vs `1` на камере
теней — Legacy **0.026**, VSM (`vsm.smrtRayCount:0`) **0.000**; RT-отражения (`render.reflectionSource:3`,
океан виден) при `vis.method:0` vs `1` — камера C **0.018**, стена **0.020** (за стеной 942 кастера
отложены GPU, роща в воде на месте).

### Гейты (полный набор, все зелёные 2026-09-05)
* `x64/Debug/test_cube.exe --log-stress` → 0; `x64/Release/test_cube.exe --log-stress` → 0.
* `cull validation PASS: 46 views + camera match CPU (…; camera 4 + 16371 deferred)` — в каждом
  профдампе стены (камерный ряд = A + deferred).
* `x64/Debug/test_cube.exe --scene-stress-gbv=20 --shadow-mode=legacy --level=data/levels/occlusion_test.json`
  → CLEAN (ассерты S6 взведены, Debug); VSM-вариант с `--set=vsm.hzbCull:1` — CLEAN (S5b.2).
* Debug / Release / Release_Editor собираются.

### Критерий приёмки
* ✅ Тени попиксельно те же при любом состоянии камерного окклюжена (Legacy 0.026 %, VSM 0.000 %).
* ✅ RT-отражения не зависят от тумблеров (AS по полному списку, 0.018–0.020 %).
* ✅ Редактор: object-id путь для indirect-объектов прослежен по коду; клик — вручную (пункт 4).
* ✅ Гейты в док, все зелёные; ассерт границы теней в Debug.
* ✅ Владение: один окклюдер на объект, картинка идентична, −0.1 мс Whole Cycle у стены K=4.

### Откат
`--set=vis.indirectQueries:1` возвращает наложение двух систем (S5 при этом остаётся); ассерты
только в Debug и ничего не меняют в поведении.
...
S6   правила границ + гейты                               [✅ 2026-09-05: владение (indirect-объекты у GPU-теста, 768 → 1 запрос у стены K=4, Whole Cycle −0.1 мс), ассерт «тени не читают камеру», RT по полному списку, все гейты зелёные]
