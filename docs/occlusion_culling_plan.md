...
| Occlusion теней | **нет** | Правило UE (§2.5). Каскады/VSM/локалы никогда не смотрят на результаты камеры. |
...
## S3a. Hardware occlusion queries + история (транскрипция `SceneOcclusion.cpp` / `FPrimitiveOcclusionHistory`)
...
### Критерий приёмки
* Статичная камера «стена» и камера теней: картинка **идентична** `vis.method:off` (дифф = пол) —
  консервативность; readout: `objectsOccluded > 0` за холмом, `chunksOccluded > 0`.
* Тонкий окклюдер (столб в `occlusion_test.json`): S3a отсекает то, что S3b (в том же прогоне, `vis.method:hzb`)
  оставляет — число объектов в readout; это и есть причина держать оба.
* `--cam-fly` через холм при `vis.queryLatency:1`: ни одного кадра с дырой; при `3` — латентность
  измерена в кадрах и лечится правилами сброса (записать числом).
* Спот за стеной: `Pass_SpotShadows` этого слота пропущен, картинка та же; при выходе из-за стены —
  тень появляется не позже первого видимого пикселя сферы.
* Стоимость на K=4: `Main_OcclusionQueries` GPU/CPU и стол `queryLatency:1` — записать; ориентир UE —
  запросы дешевле того, что они экономят, иначе троттлинг п.3.
* Тени каскадов/VSM попиксельно не зависят от `vis.method` (A/B).
...
## S3b. HZB-тестер с readback (транскрипция `FHZBOcclusionTester`) — альтернатива S3a на той же истории
...
### Критерий приёмки
* Те же, что S3a п.1, 3, 6; плюс: на тонком окклюдере S3b консервативнее S3a (оставляет больше) —
  зафиксировать число, это ожидаемо, не баг.
* Перф: `Main_VisTest` на K=4 — один dispatch, ожидание ≤ 30 мкс.
...
Жёсткие зависимости: S5 ← S4 ← S0; S3a ← S0, S1; S3b ← S2, S3a (история); S5 ← S2. S1, S2 и
S3a независимы друг от друга. **S3a/S3b не обязательны для S5**, но остаются навсегда: для
