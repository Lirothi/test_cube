...
## Ночь: чем гасить «высветление» автоэкспозиции (замер 2026-09-10)

Вопрос владельца: «какой парам у автоэкспозиции отвечает за высветление в темноте? когда солнце
ниже горизонта она уж сильно высветлить пытается».

Ручка — **`minEv100`** («Min EV100» в инспекторе объекта Camera Exposure, `--set=exposure.minEv100`):
нижняя граница адаптированного EV100, то есть потолок того, насколько камера может открыться. Кламп
живой (`exposure_solve_cs.hlsl:188` для цели и `:220` для адаптированного), но уровень возит его на
`-6` при `maxEv100 = 16` — 22 стопа, которые не зажимают ничего. Комментарий в
`PhotographicSettings.h:83` это прямо и говорит: «это ВЫКЛЮЧЕНО, а не настроено».

Замер: `wind_test`, солнце −3° (под горизонтом), камера `-101.79,23.65,42.56`, DLSS off, 10 с на
адаптацию; медиана яркости кадра без строк HUD.

| `minEv100` | медиана | доля пикселей < 5 |
|---|---|---|
| −6 … 11 — не связывает | 134.5 | 2.2 % |
| 12 | 92.2 | 8.1 % |
| **13** | **55.7** | **16.1 %** |
| 14 | 30.4 | 33.5 % |
| ручная камера, EV ≈ 15 | 14.4 | 40.1 % |

**Метр садится на ≈ EV 11.5** — примерно 3.5 стопа ярче, чем собственная фотографическая камера
уровня (f/16, 1/125, ISO 100 → EV 14.97). Показательно, что дневной кадр (солнце 20°) даёт медиану
**ровно те же 134.5**: автоэкспозиция уравнивает день и ночь, это её работа, и кламп — единственное,
что оставляет ночь ночью.

**Кламп не трогает день:** при солнце 20° `minEv100 = 13` даёт медиану 134.5 и 0.023 % пикселей
расхождения — это шумовой пол мерцания океана, тот же порядок, что между двумя одинаковыми армами.

**Оговорка про единицы.** EV100 здесь отсчитывается от произвольной линейной шкалы движка, а не от
кд/м² (см. `PhotographicSettings.h`, блок про manualEv100). Число 13 — свойство ЯРКОСТИ ЭТОГО
уровня, а не универсальная константа; на уровне с другой шкалой его надо переснять этим же способом.

**Если не хочется жёсткого клампа**, форму метра меняют две другие ручки:
`blackBucketInfluence` (UE `r.EyeAdaptation.BlackHistogramBucketInfluence`, дефолт 1) — вес самого
тёмного бакета гистограммы, опускать когда большие чёрные области тянут метр; и `lowPercentile`
(дефолт 0.15) — теневой порог метра.

**У UE то же самое**: `AutoExposureMinBrightness` / `AutoExposureMaxBrightness` («Min/Max Brightness»
в Lens|Exposure), в EV100 при ExtendDefaultLuminanceRange. Их дефолты — **−10 / 20**
(`Scene.cpp:467-468`), тридцать стопов, то есть Epic тоже возят кламп фактически выключенным и ждут,
что сцена его пропишет. Их идиома, которой у нас нет: **Min = Max полностью отключает адаптацию.**

### Диапазон EV: где он на самом деле стоит, и почему у UE другие числа (замер 2026-09-11)

Вопрос владельца: «с нашим диапазоном EV это норм сделано или что-то не так? у UE как?»

**Арифметика правильная и стандартная.** `EV100 = log2(L) + 3` (S/K = 100/12.5 = 8, форма
отражённого замера), множитель `0.18 * 8 / 2^EV`. CPU (`Ev100FromLuminance`,
`ExposureMultiplierFromEv100`) и шейдер (`exposure_solve_cs.hlsl:172`) зеркальны. UE считают цель
через тот же `kMiddleGrey = 0.18` (`PostProcessEyeAdaptation.cpp:409,573`).

**Неправильно ПОЛОЖЕНИЕ окна.** Комментарий «22 стопа, зажимает практически ничего» опровергнут —
он верен только для нижней границы. Замер `wind_test`, камера `-101.79,23.65,42.56`, солнце 20°,
медиана кадра без HUD:

| `maxEv100` | медиана | шаг |
|---|---|---|
| 13 | 226.5 | |
| 15 | 177.7 | −48.8 |
| **16 (в уровне)** | **134.5** | −43.1 |
| 18 | 122.2 | −12.4 |
| 21 | 122.2 | **0.0** |

18 и 21 совпадают до кода — значит метр свободен выше 18 и садится где-то между 16 и 18. То есть:

* `minEv100 = -6` не срабатывает НИКОГДА (ночью метр стоит на ~11.5, см. предыдущий раздел) —
  семнадцать стопов запаса, до которых ничто не доходит;
* `maxEv100 = 16` срабатывает в КАЖДОМ дневном кадре и держит картинку примерно на 1.3 стопа ярче,
  чем просил метр (134.5 против свободных 122.2).

Рабочий диапазон метра на этом уровне — примерно **EV 11.5…17**, пять с половиной стопов. Честное
окно здесь — что-нибудь около 11…18, а не −6…16.

**Числа UE копировать нельзя.** Их дефолты −10 / 20 (`Scene.cpp:467-468`), но якорь другой:
`LuminanceToEV100 = log2(L / LuminanceMax)` (`RenderUtils.h:680`) с
`LuminanceMax = 0.78 / LensAttenuation` (`PostProcessEyeAdaptation.cpp:224-237`), а
`r.EyeAdaptation.LensAttenuation` по умолчанию 0.78 — их же справка: «Set this number to 0.78 for
lighting to be unitless (1.0cd/m^2 becomes 1.0 at EV100)». Значит у них 1.0 кд/м² = EV100 0, у нас
= EV100 3. **Один и тот же свет читается на три стопа выше в наших числах**; их −10 / 20 — это наши
−7 / 23. Оттуда же берётся «1.2» из блока про kMiddleGrey: это их ЛЕГАСИ-режим LensAttenuation 0.65
(0.78 / 0.65 = 1.2), а не текущий дефолт.

Отдельная их ручка, которой у нас нет: `r.DefaultFeature.AutoExposure.ExtendDefaultLuminanceRange`.
Выключенная, она жёстко ставит `LuminanceMax = 1.0`, и тогда Min/Max Brightness читаются как сырые
кд/м², а не как EV — это старый режим, из-за которого одни и те же поля в разных проектах значат
разное.
