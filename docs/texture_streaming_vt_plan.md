...
(7) **гистерезис роста бюджета = margin, не temp + margin** (`AsyncTextureStreaming.cpp:654-665`):
у UE temp-текстуры живут в пуле и пул — сотни МБ, у нас ринг — отдельный UPLOAD-хип, а пул в
тестах 8–40 МБ, и порог 55 МБ намертво держал бюджет на 0 после первого сжатия (владелец
поймал в dev-окне: «меняю пул — ничего не происходит»). Проверено `--sweep=streaming.poolSizeMB:
-1,12,40,8,-1`: бюджет 16891 → 7 → 35 → 3 → 16891, занято 11 → 6 → 11 → 2 → 11.
(8) **Чанкованный меш = бокс на чанк** (как компонент ландшафта UE), не один AABB объекта: остров
целиком давал расстояние ≈ 0 из любой точки над ним, и песок просил все мипы даже при отлёте
(владелец: «так остров же разбит на чанки»). Ключ `lastSeen` для чанка = адрес объекта + индекс.
Проверка `--cam-fly=-40,0` с `--set=streaming.dumpRows:1` (дамп таблицы в лог): песок на старте
`needed 11/11`, через 13 с (520 м) `7/7`; `wanted/budgeted/resident` при пуле −1 остаются 11 —
`TryKeepMips` держит приехавшее, пока влезает, сброс только под давлением пула. В таблице
dev-окна добавлена колонка `needed vis/hid` (UE perfect wanted до бюджета).
...
**Статус (2026-09-22): СДЕЛАНО.** `MipBiasFade.h` — транскрипция `FMipBiasFade` (SetNewMipCount /
CalcMipBias, порог возраста 0.5 с, 0.3 / 0.1 с НА МИП); живёт на записи реестра
`TextureStreaming::Entry`, взводится при adopt (`FinalizeStreaming`-точка) и при регистрации
(`CreateTexture`-точка); каждый кадр `UpdateFades_` → `Texture2D::SetMinLodClamp` переписывает
SRV на месте с `ResourceMinLODClamp = bias`, возврат к 0 бампает `srvGeneration_` (RT-bindless
подхватывает незаклампленную копию). Провайдер `tex` (Σ резидентных мипов реестра) + `tex.ret`,
`render::TickMemoryReport` возвращён в `App.cpp`. Ручки `streaming.mipFade / mipFadeIn /
mipFadeOut` (`--set`), чекбокс «Mip fade» во вкладке Streaming (персистится в JSON).

Отступление: **фейд только на stream-in.** UE фейдит ВНИЗ до сброса мипов (`bWaitForMipFading`:
stream-out ждёт, пока bias дойдёт до цели), у нас adopt уже поставил ресурс без верхних мипов —
клампить дальше некуда. Сброс мипов = поп в сторону размытия, по построению у далёких/скрытых.

Приёмка: серия `--sweep=streaming.forceMips:4,12,12,…` (14 шотов через 0.25 с, пляж),
mean|shot_k − final|: без фейда `6.58 → 0.11` одним шагом (max шаг 6.54), с фейдом
`6.57 6.37 5.23 3.38 1.80 0.86 0.11` — переход растянут на ~1.5 с, max шаг 3.17; середина фейда
глазами — мягкое проявление, не скачок. `mem:` содержит `tex 38.3 MB` при `used 38` в
`[texstream]` (одна функция), `tex.ret` дренируется в 0. Три конфига собраны.
...
| `FMipBiasFade` через bias семплера | `ResourceMinLODClamp` SRV на кадр; фейд только на stream-in | SRV и так копируется каждый кадр; сброс мипов у нас раньше фейда |
