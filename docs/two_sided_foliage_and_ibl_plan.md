...
transmission lobe. The transmission lobe samples the directional shadow with the visible normal flipped
(`-N`) and the back cosine, offsetting the receiver toward the sun-facing side — the scoped
transmission shadow bias this step anticipates. Without it the leaf's own front face self-shadows the
transmission to black; with it a genuine occluder between the sun and the leaf still darkens the
sun-facing side and removes transmission. No root-signature, descriptor, or GBuffer changes. Verified
on a temporary two-palm fixture (a foliage frond slot beside an unchanged DefaultLit palm sharing the
same leaf textures) under front and back sun: front-lit foliage is visually identical to DefaultLit
...
**Result:** `shaders/pointlight_cs.hlsl` and `shaders/spotlight_cs.hlsl` now reuse the exact F4
`EvalFoliageBRDF` helper — no duplicated BRDF. Each decodes the shading model from `GBAux.b` once per
pixel and, for foliage, adds the front Lambert+GGX lobe (gated on `NdotL > 0`) plus the back
transmission lobe (gated on a non-zero payload); every lobe is multiplied by that light's own
attenuation (radius for point, distance × cone for spot) and its shadow visibility. As in F4 the
transmission lobe samples the light's shadow with the visible normal flipped (`-N`) so a back-lit leaf's
...
are equally back-lit): the foliage palm's camera-facing fronds gain a warm tinted-green transmission
glow under both point and spot while the DefaultLit palm shows only the silver sky-spec leak; cone/radius
