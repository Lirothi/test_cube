...
* «Слегка повернул камеру» — **решётка прямоугольных плиток по всему небу** (поза
  ...
  **Тот же обратный путь живёт в `ssr_temporal_cs.hlsl` (океанский режим `planeReproject`,
  строки ~167–182)** — та же латентная решётка при длинной истории; в его CB нет текущей `viewProj`,
  правка требует C++. Бисект-армы, снятые после проверки: кап октав шума (не при чём, оставлен как
  был), поворот+варп решётки детали (не при чём, снят).

**Не сделано / открыто:** обратный путь репроекции в `ssr_temporal_cs` (см. выше); C4 (облака в RT-отражениях и IBL-захвате); тень облаков на ОКЕАНЕ
