...
`ocean_surface.hlsl` / `ocean_surface_surf_sim.hlsli`: `CombineVolumetricFog` по глубине поверхности
...
* B4: в обоих океанских путях (`ocean_surface.hlsl`, `ocean_surface_surf_sim.hlsli`) отражения,
...
* `fog_common.hlsli`: `FogSkyAlongView(...)` — один источник для всех четырёх потребителей тумана
  (`compose_cs.hlsl:448`, `ocean_surface.hlsl:2248`, `ocean_surface_surf_sim.hlsli:1360`,
...
### C4. Облака в отражениях и IBL — СДЕЛАНО (2026-09-12), не закоммичено
Постановка была: RT-отражения при промахе TLAS сэмплируют небо: добавить облака по лучу с
`ReflectionRaySampleMaxCount 80/24`; IBL-захват B4 включает облака (низкая частота обновления).
**Сделано по пути UE, а не маршем на луч:** UE рендерят облако в real-time sky light capture
(`VolumetricCloudRendering.cpp:2257 IsReflectionRendering`, `:2266-2280` — `SampleCountMax =
min(96·ReflectionViewSampleCountScale, r.VolumetricCloud.ReflectionRaySampleMaxCount 80)`, тень
`min(10·scale, 24)`), а все отражения — RT-промахи и хиты, вода, стекло — читают этот кубмап.
* **`cloud_capture_cs.hlsl`** компонует облака в radiance-куб B4 (128², 8 мипов) НА МЕСТЕ внутри
  `Main_SkyEnvironment`, между захватом SkyView и фильтром проб: `radiance = radiance·T + L`. Фильтр
  дальше сам разносит их в specular/irradiance; композ (RT-fallback), RT hit env, океан вне экрана,
  стекло и туман (картинка неба) читают тот же куб — ничего из потребителей не трогалось.
* Хук: `SkyEnvironmentOverlay` (SkyAtmosphere.h) → `SkyAtmosphere::BuildEnvironment(..., overlay)`:
  dirty окружения |= dirty оверлея | появление/исчезновение оверлея; продюсер
  `VolumetricCloud::MakeEnvironmentOverlay` (ключ последнего захвата коммитится в serial builder);
  шум облаков регистрируется ДО окружения (`SceneRenderer_Graph.cpp`), окружение зависит от него.
  CB захвата = `CloudCB` + `SkyAtmosphereParameters` + `SkyViewFrameData` одним блобом
  (`CLOUD_WITH_SKY_CB` в cloud_common, `SKY_CB_EXTERNAL` в sky_atmosphere; размер сверяется с
  рефлексией при Prepare). Скоуп `SkyEnvironment.CloudCapture`.
* Марш вынесен в **`cloud_march.hlsli`** (общий для трассы и захвата; трасса — тот же код, DXIL
  того же размера, отличается порядком инструкций). Сегментная интеграция AP вынесена из
  `sky_lut_aerial_cs` в `SkyIntegrateSegment` (sky_atmosphere.hlsli): A/B кадра одним бинарём с
  HEAD-шейдерами (облака выкл, детерминированно) — 1.28 % / 0.43 % пикселей при поле same-state
  1.23 % / 0.30 % → в пределах пола.
* Дельты от UE (все сознательные, в шапке шейдера): проба на уровне моря ПОД КАМЕРОЙ (глобальная
  проба неба ставила бы облака мирового нуля над головой); марш детерминированный (offset 0.5, без
  джиттера и истории — куб не должен мерцать между перезахватами); 80 view-сэмплов независимо от
  бюджета главного вида, тень — счёт уровня; AP до фронта облака — сегментная интеграция тех же
  LUT (8 шагов) вместо камерного объёма (горизонтальный ряд куба = дальний цвет тумана, B6 —
  дальнее облако без воздуха перед ним было бы тёмной полосой); планета вместо depth-буфера.
* Обновление: при смене настроек облаков, шума или distant-света; иначе не чаще раза в 4 кадра
  и только если ветер сдвинул поле или проба ушла > 10 м; статичная сцена с замороженным ветром
  не перезахватывает (правило B4). Каденция подтверждена Debug-логом: rebuild на кадрах 1, 5, 9, …
  (кадры 0 и 1 — distant-свет появился после первого захвата, он читается прошлокадровый).
* **Стоимость** (Release 1440p native, `--profdump`, HEAD-уровень coverage 1.0, живой ветер):
  `Pass_SkyEnvironment` 0.295 мс за перезахват, из них `SkyEnvironment.CloudCapture` 0.211; раз в
  4 кадра → ≈ 0.07 мс/кадр амортизированно. (Там же: `Pass_CloudTrace` 1.28 мс при полном покрытии
  и 128 шагах — утренние 0.51 мс были при частичном; стоимость трассы растёт с покрытием.)
* **Проверка глазами** (поза взгляда вниз `-105.97,8.40,21.98 / 0.2138,0.1523,-0.0338,0.9640`,
  на экране облаков нет, `cloud.enabled` 1/0 одним бинарём изолирует куб): вода отражает светлое
  облачное небо вместо синего (`c4_down_00/01`, 98 % px > 2); при `overcast 1` IBL и тень C3 гасят
  песок (`c4_down_oc_00`). **Океан под сплошной палубой по-прежнему блестит солнцем — тень облаков
  на океане не заведена (старый открытый пункт, с оверкастом бросается в глаза).**
* Гейты: Debug / Release / Release_Editor собраны; check_shaders 85/85; check_logging 0; Debug-прогон
  с облаками exit 0 без ERROR/FATAL; log-stress Release 0 и Debug 0; GBV
  `--scene-stress-gbv=64 --scene-stress-sky --gbv-mode=unguarded --no-streamline --set=cloud.enabled:1`
  на Debug — 64 итерации, exit 0, 0 ERROR/FATAL (patch mode 2, debug-device QI ok).
* Ловушка: `Set-Content -Encoding utf8` (PowerShell 5.1) пишет BOM → dxc «Illegal character», первый
  A/B «HEAD-шейдер» рендерил без AP и врал на 99.99 %; подмена файлов только `cmd /c git show > file`.

### Что сделано (2026-09-12): C1 + C2 + C3 одним инкрементом (закоммичено 57e5ae2 и далее), C4 выше
...
* плотность — НЕ транскрипция (у UE материал): Schneider 2015 — Perlin-Worley 128³ RGBA8 + Worley 128³ (октавы капятся на ≥ 4 текселя на ячейку)
...
* blue noise → IGN с R2-сдвигом по кадру; чекерборда 2×2 нет — трассируем каждый полурез-тексель с
  джиттером старта и копим темпорально. **Резолв — НЕ транскрипция `VolumetricRenderTarget.usf`**:
  от UE в нём только репроекция ПО ГЛУБИНЕ ОБЛАКА tAP (`:205-218`; у неба velocity = только поворот
  камеры); экспоненциальное накопление, 3×3-клэмп и stillness — форма нашего `ssr_temporal_cs`
  (UE TemporalAA.usf). Сам UE в режиме 0 трассирует один из четырёх текселей чекерборда за кадр,
  берёт его как есть, остальные три — репроецированная история без смешивания и клэмпа (AABB-клэмп
  `ReprojectionBoxConstraint` выключен по умолчанию), а стохастический шум гасит основной TAA/TSR
  на полном разрешении; у нас своего TAA на композите нет (DLSS — только когда включён), а шум за
  кадр при бюджетном числе сэмплов в разы выше их 768, поэтому накопление живёт здесь;
...
* «Пиксельная пляска на тонких участках, сильнее при меньшем числе шагов» — дыра в резолве, не
  свойство марша. Тексель, чей луч в этом кадре ПРОМАХНУЛСЯ мимо тонкого клока (T > 0.99), нёс
  глубину = дальний конец пути (до 50 км) вместо 3 км облака; резолв сравнивал её с глубиной истории
  по порогу 10 %, который я ошибочно взял из `VolumetricRenderTarget.usf:431` — там сравниваются
  глубины СЦЕНЫ (геометрия, «облако над деревьями»), не облака с облаком, и вся эта логика у UE
  выключена, когда всё дальше 5 км (`MinimumDistanceKmToDisableDisoclusion`); видел разы, считал
  разокклюзией и сбрасывал историю — сырой сэмпл каждый кадр, и тем чаще, чем меньше шагов. Правки
  `cloud_temporal_cs.hlsl` (по поведению UE — историю по глубине облака не выбрасывать; сами правила
  наши): промах репроецируется глубиной
  истории в этом же текселе; тест глубин только когда облако в ОБОИХ кадрах и широкий (0.5..1.0 от
  дистанции — «фронт» облака есть взвешенное среднее сквозь слой толщиной в километры); канал глубины
  на промахах наследует глубину истории; stillness как в `ssr_temporal_cs` (клэмп ослабляется к сырой
  истории на 0.5·stillness, вес кадра делится на 1 + 2·stillness). Замер одним бинарём, та же поза,
  coverage 0.85, native, `--shot-count=8 --shot-step=0` (кадры через 0.1 с при неподвижной камере,
  σ по пикселю в 8-битных кодах, без HUD-полосы):

  | | σ средняя | σ худшие 10 % | пикселей σ > 1 | пикселей σ > 3 |
  |---|---|---|---|---|
  | до | 0.302 | 1.227 | 4.49 % | 0.326 % |
  | после | 0.142 | 0.570 | 0.59 % | 0.011 % |

  Остаток — сходимость стохастического марша при неподвижной камере (ручки `historyWeight`,
  `viewSampleCountMax`); в движении вес кадра возвращается к 1 − historyWeight, шумнее по замыслу
  любого TAA. Снимки `logs/c2_flk_before_00..07.png`, `c2_flk_after_00..07.png`,
  скрипт σ — `series_sigma.py` (scratchpad сессии).

* «Диагональные полосы из-за истории» (поза `-105.97,8.40,21.98 / -0.1240,-0.1300,-0.0164,0.9836`)
  — **не история и не шум старта: это собственные тени комков детали вдоль солнца** (штрихи в обеих
  позах сходятся к образу солнца). Честная хронология:

  1. Первая атрибуция — шум старта марша: IGN — штрих, и я сдвигал его каждый кадр на R2 × 64 px,
     история размазывала. Замена на temporal IGN (паттерн неподвижен, по кадру — золотое сечение)
     **оставлена ради мерцания** (σ серии 0.142 → 0.166, худшие 10 % 0.570 → 0.498, σ > 3 → 0.000 %;
     белый хеш втрое хуже: 0.475), но полосы владелец увидел на месте («2 кропа почти идентичны»).
  2. Перебор одним бинарём (`--sweep`), метрика `band_metric.py` = доля энергии градиента в секторе
     ±15° к направлению полос (у гладкого облака без тени 30.7 %) + глаза. **Не влияют:** шаги марша
     128/256, дистанция тени, базовый тайл, бикубическая погода, детальный шум 32³ → 128³, вертикальное
     сжатие базы, поворот + варп решётки детали. **Убирают:** `detailStrength 0`, `layerHeightKm 2.5` и
     `shadowTracingDistanceKm 0.001` (теневой марш выключен) → полосы исчезают полностью.
  3. Это не квантование марша: `shadowSampleCount` 2 / 6 / 24 → 31.3 / 34.7 / 36.3 % — чем точнее
     марш, тем резче гребёнка. Физика честная: точечное солнце + непрозрачный комок детали = жёсткая
     теневая колонна длиной в путь солнца через слой; слой 0.5 км при низком солнце — гребёнка.
  4. Рычаги заполнения тени, тот же бинарь, та же поза (пик сектора / средняя яркость кропа):

     | вариант | пик 0–15° | яркость | глазами |
     |---|---|---|---|
     | как было (2 октавы MS, occlusion 0.5) | 34.7 % | 79.8 | гребёнка |
     | конус теневого марша 8.5° (Schneider) | 34.5 % | — | без изменений, +шум по кадру → снят |
     | `msOcclusion` 0.2 / 0.05 | 31.4 / 30.0 % | 95 / 99 | бледнее, при 0.05 — плоская серая пелена |
     | 3 октавы MS, occlusion 0.5 / 0.3 / 0.2 | 32.5 / 31.1 / 30.2 % | 88 / 95 / 99 | бледнее, есть |
     | `extinctionScale` 0.02 / 0.01 | 31.8 / 36.2 % | 92 / 98 | светлее, есть |
     | `skyLightBottomOcclusion` 0 / 1 | 32.5 / 33.9 % | 92 / 79 | есть |
     | **детальный LOD теневого марша (ниже)** | **24.8 %** | 80 | **гребёнки нет, комки затеняют свой бок** |

     Дефолты UE-материала: октав MS **0**, contribution 0.5, occlusion 0.5
     (`MaterialExpressionVolumetricAdvancedMaterialOutput.h:64-72`) — наши 2 октавы уже больше их;
     все ручки оставлены как были.
  5. **Правка** (`cloud_trace_cs.hlsl`, `CloudSampleAt(…, detailWeight)` в `cloud_common.hlsli`):
     сэмпл теневого марша дальше двух фич детали (фича = ячейка Worley = четверть `detailTileKm`)
     эрозирует средним детали (0.48, измерено на numpy-порте шума), ближе — точечным значением; вес
     линейный между одной и двумя фичами. Это наш модельный выбор, аналог того, что UE отдают
     материалу как `ShadowSampleDistance` (`VolumetricCloud.usf:1096`, `MaterialTemplate.ush:2568`):
     непрозрачную колонну октавы не заполнят (степень того же нуля), заполняет её свет, рассеянный
     соседними освещёнными комками, и среднее детали — та плотность, которую он видит. По дистанции,
     не по длине сегмента, чтобы рост `shadowSampleCount` уточнял тень, а не возвращал гребёнку.
     Лишних выборок нет (тот же fetch, один lerp). Снимки: `c2_bands_shx_zoom.png` (было),
     `c2_bands_dlod2_zoom.png` (стало), `c2_look_shx.png` / `c2_look_dlod2.png` (поза взгляда вверх,
     coverage 0.85), `c2_bd_noshadow_01.png` (контроль без теневого марша), свипы `c2_bd_*`.
  6. **Промежуточные правки этой охоты — что оставлено, что снято.** Оставлено: марш тени, ограниченный
     выходом из слоя вдоль солнца (первый сэмпл 16 м от точки вместо 208 м для слоя 0.5 км при солнце
     25° — оправдано само по себе); детальный шум 128³ с капом октав ≥ 4 текселя на ячейку (шум без
     собственного алиасинга); бикубическая (B-spline) выборка погоды 512² (гладкие рампы покрытия;
     на полосы нейтральна, отдельно не мерилась); ручка `baseVerticalTiles` (сжатие базового шума по
     вертикали, в свипе меняла форму комков). Снято: поворот + варп решётки детали (нейтрально,
     недоказано), конус теневого марша (см. таблицу). Старый шум старта: `c2_bands_{00,01}`,
     `c2_bands_white_*`, `c2_bands_gold_*`, серии `c2_flk_white_*`, `c2_flk_gold_*`.

* «Слегка повернул камеру» — **решётка прямоугольных плиток по всему небу** (поза
  `-105.97,8.40,21.98 / -0.0053,0.1579,0.0009,0.9874`, владелец: «видно только при включённой
  темпоралке»). **Это репроекция резолва, не плотность и не шум.** Доказательства одним бинарём
  (кроп `700,60–1500,460`, снимки `c2_grid_*`):

  | арм | решётка |
  |---|---|
  | `temporal 0` (сырой кадр) | нет |
  | среднее 8 сырых кадров без резолва (`c2_grid_rawmean`) | нет → в детерминированном сигнале её нет |
  | `historyWeight` 0.9 / 0.5 / 0.0 | есть / нет / нет |
  | `viewSampleCountMax` 64/128/256, `weatherTileKm` 40/80, `baseTileKm` 6/12, `baseVerticalTiles` 1/12, `detailStrength` 0.35/0, кап октав шума выкл, поворот решётки детали | без изменений |
  | другое рысканье (yaw 90°, −72°) | нет (нет мелкой текстуры на нужном масштабе) |
  | **история берётся из того же текселя (репроекция обойдена)** | **нет** |

  Механизм: точка облака восстанавливалась из центра текселя через `invViewProj` (ndc z 0.5 — точка
  в 0.2 м перед камерой, из неё вычитается позиция камеры ~100 м, нормализация) и проецировалась
  обратно через `prevViewProj`; у неподвижной камеры матрицы побитно равны, но обратный путь даёт
  не центр текселя, а сдвиг в сотые доли текселя с узором округления. Билинейная выборка истории с
  таким сдвигом каждый кадр при `historyWeight 0.9` (~30 пересэмплов) размывает там, где сдвиг у
  половины текселя, и оставляет резким там, где ноль — плитки с прямыми границами. Правка
  (`cloud_temporal_cs.hlsl`): та же точка через ОБЕ матрицы, `prevUv = uv + (prev − cur)` —
  формулировка motion vector; у неподвижной камеры разность — ноль побитно. Побочный выигрыш:
  серия σ на позе пляски (8 кадров, coverage 0.85) 0.166 → **0.125** средняя, худшие 10 % 0.498 →
  **0.417**, пикселей σ > 1: 0.157 % → **0.000 %** (пересэмпл истории у неподвижной камеры больше
  не размывает и не шумит). Полосы на прежней позе не вернулись (пик 23.6 %).
  **Тот же обратный путь жил в `ssr_temporal_cs.hlsl` (океанский режим `planeReproject`) — исправлен
  тем же способом (2026-09-12):** в CB добавлена текущая `viewProj` (`SsrTemporalConstants::viewProj`,
  хэндл + запись в `SceneResourceBootstrapper`, продюсер в `SceneRenderer_Reflections.cpp`), motion =
  разность двух проекций точки плоскости. На воде решётка глазами не наблюдалась ни до, ни после
  (отражение без мелкой текстуры нужного масштаба), замер одним прогоном на арм, поза решётки,
  8 кадров: длинная история `ssr.temporalBlend 0.03` — σ средняя 0.0574 → 0.0512, худшие 10 %
  0.492 → 0.433, пикселей σ > 1: 0.452 % → 0.376 %; дефолт 0.125 — без изменений (0.0594 → 0.0598),
  как и должно быть для короткой истории. Один прогон на арм — направление согласуется с облаками,
  но за пол не выдаётся. Три конфигурации собраны, check_shaders 84/84. Снимки `ssr_{before,after}_{long,def}_0*`.
  Бисект-армы облаков, снятые после проверки: кап октав шума (не при чём, оставлен как был),
  поворот+варп решётки детали (не при чём, снят).

* «Почему coverage не даёт полный оверкаст» (владелец, 2026-09-12). Потому что `coverage` — ПОРОГ
  по полю погоды: `remap(weather.r, 1 − k, 1, 0, 1)`, при k = 1 поле проходит как нарисовано, а
  процедурная карта после растяжки контраста `0.35..0.70` на треть площади равна нулю. Дыры — только
  карты: базовая форма до нуля не доходит (нижняя граница её remap ≤ 0, она лишь истончается; моё
  первое объяснение владельцу про «второй потолок в базе» было неверным). Schneider закрывает небо
  РИСУЯ карту погоды; у нас она процедурная, поэтому добавлена ручка **`overcast`** (0..1, дефолт 0;
  JSON `overcast`, `--set=cloud.overcast`, Inspector → Volumetric Cloud → Shape → «Overcast»,
  CB `cloudShadowMap2.w`): `weatherCoverage = saturate(weather.r + overcast)` перед порогом, и ничего
  больше. Первая версия поднимала ещё и базу к 1 (`lerp(base, 1, o)`) — палуба без единой складки
  (постоянная плотность нигде не эрозирует), снята. Замеры (закоммиченный дневной уровень из
  `git show HEAD`, поза `-105.97,8.40,21.98 / -0.0053,0.1579,0.0009,0.9874`, `c2_oc2_*`):

  | | 0 | 0.5 | 1 |
  |---|---|---|---|
  | transmittance-вид, среднее по небу (0 = непрозрачно) | 95 | — | **11** (чёрное небо: слой закрыт) |
  | coverage-вид, среднее | 182 | — | 210 |
  | тождество при 0 против сборки без ручки, тот же уровень | 0.00 % px > 2 | | |

  **Что даёт модель при 1:** закрытый слой с рябью, но его низ освещён только distant sky light
  (синий) с затуханием `skyLightBottomOcclusion` — солнце сквозь OD ≈ 10 ни наша, ни UE'шная
  аппроксимация MS не проносит, поэтому днём палуба снизу сине-серая и по яркости близка к небу
  (читается как дымка), на закате — ровная пурпурная. `skyLightBottomOcclusion` 0.5 / 0 / 1 меняет
  низ слабо (RGB 88/112/157 → 91/119/167 → 81/97/135). Настоящий белёсый оверкаст = транспорт
  солнца сквозь палубу (третья октава MS даёт ~8 % через OD 2.5, через 10 — ничего) — открыто.
  Под палубой океан по-прежнему блестит солнцем: тень облаков на океан не заведена (старый пункт).

* **Тень облаков на океане — СДЕЛАНО (2026-09-12, владелец: «делай тени на океане»).** У воды не было
  солнечной тени вовсе (`shadowAttenuation` = константа 1), поэтому под сплошной палубой море блестело,
  а песок рядом гас. Теперь обе поверхности океана читают карту C3 тем же хелпером, что lighting и туман
  (`cloud_shadow_common.hlsli`): `CloudSunVisibility(worldPos)` заполняет слот `light.shadowAttenuation`
  (спекуляр/блики и пена его уже умножали), а в теле воды (`GetOceanColor`) свет = солнце × тень +
  **измеренная sky irradiance** `SkyFillRadiance(li.normal)` (t19, E/π, куб с облаками C4 — под палубой
  темнеет сам). Пол «0.2 × цвет солнца», который был у диффузного члена вместо неба, снят: в
  солнце дельта −0.5 % (пол и небо здесь почти равны), в тени вода получает свет неба, как суша. Две
  первые версии (масштаб цвета солнца целиком → «дико тёмные»; затем пол 20 %) отброшены.
  Плюмбинг: t21 `CloudShadowMap` (RS 22), поля `cloudShadowViewProj` / `cloudShadowParams` в хвосте
  `OceanCB` (хэндлы по имени), `OceanRenderable::SetCloudShadow` из билдера `Main_Transparent` (там
  `ShadowBuilt()` уже закоммичен), карта объявляется PIXEL там же (её rest — NON_PIXEL).
  Замер, фиксированная экспозиция (`exposure.autoExposure:0`), `cloud.shadowMap` 1/0 одним бинарём,
  дневной HEAD-уровень coverage 1.0, ЛИНЕЙНЫЙ свет (средние по кодам через тонмап врут):

  | область | тень вкл / выкл | доля в тени |
  |---|---|---|
  | песок (горизонт / вниз) | 0.090 / 0.341, 0.093 / 0.341 | 0.26 / 0.27 |
  | вода (горизонт) | 0.118 / 0.219 | 0.54 |
  | вода (взгляд вниз) | 0.061 / 0.182 | 0.33 |

  Владелец: «вода не так реагирует на тень, как плотная геометрия: гаснет спекуляр, сама вода сильно
  не темнеет». Матчасть (Mobley, Light and Water): яркость моря = френелевское отражение неба/солнца
  + water-leaving radiance ≈ R_rs·E_d (R_rs 0.5–2 % для чистой воды); тень облака убирает прямое
  солнце (блики), а толща светится диффузным E_d, который под рваной облачностью остаётся 30–60 % от
  ясного — подсветка ярких боков и низов облаков, которой в нашем кубе нет (низы облаков освещены
  только distant sky light, открытый пункт). Поэтому **тень на толщу дозируется ручкой
  `oceanBodyShadow`** (0..1, дефолт 0.35; JSON / `--set=cloud.oceanBodyShadow` / Inspector →
  Volumetric Cloud → Shadow map → «Ocean Body Shadow»; едет в `cloudShadowParams.z`, новых полей CB
  нет): `bodyShadow = lerp(1, T, k)` на deep scatter / SSS / направленный диффуз; блики и пена берут
  T целиком; sky irradiance добавляется всегда. Замер той же парой (линейный свет, доля в тени):

  | `oceanBodyShadow` | вода, взгляд вниз | вода, горизонт | песок (не зависит) |
  |---|---|---|---|
  | 0 | 0.69 | 0.89 | 0.33 |
  | **0.35** | **0.57** | **0.78** | 0.32 |
  | 1 | 0.38 | 0.60 | 0.32 |

  Снимки `c7_{down,grid}_{00,01,02}` (0 / 0.35 / 1), `c6_*` (k = 1 до ручки); под `overcast 1` море без
  бликов (`c5_down_01`). Когда низы облаков получат транспорт солнца, ручку можно будет поднять к 1.
* **Поверхности океана переименованы по сути (владелец: «завязывать с легаси»).**
  `ocean_surface_legacy.hlsli` → **`ocean_surface_surf_sim.hlsli`** (поверхность в работе, дефолт,
  `--ocean-surf-sim-shore`; старое `--ocean-classic-shore` принимается), современное тело вынесено из
  `ocean_surface.hlsl` в **`ocean_surface_runup.hlsli`** (`--ocean-runup-shore`), `ocean_surface.hlsl` —
  переключатель. Общие функции обеих поверхностей (50 функций, ~430 строк, идентичные текстуально,
  плюс `ClipMapVertexInternal` и константы `kLodThreshold`/`kNormalScale`) вынесены в
  **`ocean_surface_common.hlsli`**, туда же карта тени и `CloudSunVisibility`; `kGradientMaxKeys`
  остался в файлах (его использует `struct Gradient` до точки вставки). Приёмка выноса: DXIL VS обеих
  пермутаций побитно идентичен, PS — те же 2368/2570 инструкций как мультимножество, отличается только
  внутренняя нумерация `createHandle` (t21 объявлена в другом месте списка). Функции с одинаковым именем
  и РАЗНЫМ телом (14: SampleSceneDepth, GetFoamData, Specular, Reflection, Refraction, GetOceanColor, …)
  остались в файлах — их не сливать, не проверив, что обе поверхности хотят одного и того же.

**Не сделано / открыто:** белёсый низ оверкаста (транспорт солнца сквозь палубу, см. выше); темпоральный пол on/on статичного кадра не мерился (правило
