...
**Критерий приёмки:** камера «роща под солнце»: лучи видны глазами (side-by-side off/on); камера
теней: паритет с off ≤ 0.5 % (туман тонкий, `density 0.004`); стоимость scatter+integrate ≤ 0.25 мс
@1080p (профдамп ×2). **Дефолт `fog.volumetric` → 1 после приёмки.**
