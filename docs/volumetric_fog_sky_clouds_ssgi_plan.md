...
* «Диагональные полосы из-за истории» (поза `-105.97,8.40,21.98 / -0.1240,-0.1300,-0.0164,0.9836`)
  — шум старта марша. IGN — это ШТРИХ (значения идут линиями, кроп без истории показывает его как
  мелкую диагональную сетку), а я сдвигал паттерн каждый кадр на R2 × 64 px по диагонали; длинная
  история при неподвижной камере усредняла движущийся штрих в широкие полосы вдоль вектора сдвига.
  Перебор одним бинарём, та же серия σ (8 кадров, поза пляски, coverage 0.85) и кроп полос:

  | шум старта | σ средняя | худшие 10 % | σ > 1 | σ > 3 | полосы |
  |---|---|---|---|---|---|
  | IGN, сдвиг R2 × 64 px/кадр | 0.142 | 0.570 | 0.59 % | 0.011 % | есть |
  | белый хеш (пиксель, кадр) | 0.475 | 1.291 | 8.46 % | 0.087 % | нет |
  | **IGN + золотое сечение × кадр (temporal IGN)** | 0.166 | **0.498** | **0.16 %** | **0.000 %** | нет |

  Оставлен temporal IGN: пространственный паттерн неподвижен (нечего размазывать, штрих гаснет с
  историей), а по времени на пиксель — золотая последовательность, лучшая 1D-последовательность
  (ошибка ~1/N против 1/√N у случайной, отсюда белый шум втрое хуже). UE берут blue-noise текстуру
  по StateFrameIndexMod8; у нас такого ассета нет — при желании 64² void-and-cluster на CPU при
  загрузке + тот же золотой сдвиг по кадру = их схема. Снимки `c2_bands_{00,01}` (было),
  `c2_bands_white_*`, `c2_bands_gold_*` (+ `_crop`), серии `c2_flk_white_*`, `c2_flk_gold_*`.
