        ...
        EditorAssetImportStatus& statusOut)
    {
        ...
        if (!projectIsFile)
        {
                 ...
                 it.increment(ec))
            {
                ...
                if (!it->is_regular_file(fileEc)) { continue; }
                const std::string ext = ToLower(it->path().extension().string());
                // W7.1b: our baked <name>.mesh.bin is regenerated by the mesh-asset bake (like the
                // .mesh.json), NOT a copied import output — it isn't recorded in the manifest, so it
                // must not count toward the tracked-output comparison (else every migrated mesh reads
                // as Incomplete/MISSING).
                const std::string lowerName = ToLower(it->path().filename().string());
                const bool isMeshBin = lowerName.size() >= 9 &&
                    lowerName.compare(lowerName.size() - 9, 9, ".mesh.bin") == 0;
                if (isMeshBin || (ext != ".dds" && !IsCopiedImportExtension(ext))) { continue; }
                std::error_code relativeEc;
