...
public:
    ...
    void RebuildHandles(RenderableObject& owner) override
    {
        if (Material* material = owner.GetGraphicsMaterial())
        {
            ...
            fogParams0Handle_ = material->ComputeCBFieldHandle(0, "fogParams0");
            fogParams1Handle_ = material->ComputeCBFieldHandle(0, "fogParams1");
            fogParams2Handle_ = material->ComputeCBFieldHandle(0, "fogParams2");
            fogDebugViewHandle_ = material->ComputeCBFieldHandle(0, "fogDebugView");
            ...
            cloudShadowViewProjHandle_ = material->ComputeCBFieldHandle(0, "cloudShadowViewProj");
            cloudShadowParamsHandle_ = material->ComputeCBFieldHandle(0, "cloudShadowParams");
    ...
    void UpdateMainCB(RenderableObject& owner, Renderer* renderer, const Camera& camera, uint8_t* cbData) override
    {
        ...
        UpdateUniform(owner, fogParams0Handle_, material, owner_.GetFogParams0(), cbData);
        UpdateUniform(owner, fogParams1Handle_, material, owner_.GetFogParams1(), cbData);
        UpdateUniform(owner, fogParams2Handle_, material, owner_.GetFogParams2(), cbData);
        UpdateUniform(owner, fogDebugViewHandle_, material, owner_.GetFogDebugView(), cbData);
...
private:
    ...
    Material::CBFieldHandle fogParams0Handle_{}, fogParams1Handle_{};
    Material::CBFieldHandle fogParams2Handle_{};
    Material::CBFieldHandle fogDebugViewHandle_{};
    Material::CBFieldHandle fogVolumeParamsHandle_{}, fogVolumeZParamsHandle_{};
    Material::CBFieldHandle cloudShadowViewProjHandle_{}, cloudShadowParamsHandle_{}; // plan C3
...
bool OceanRenderable::RecordGraphics(Renderer* renderer, ID3D12GraphicsCommandList* cl, RenderContext& ctx, const Camera& camera, uint8_t* cbData)
{
    ...
    std::array<D3D12_CPU_DESCRIPTOR_HANDLE, 22> srvs{};
    size_t srvCount = 0;
    ...
    // t21: plan C3, the cloud shadow map (Main_Transparent declares it PIXEL-readable on the frames
    // it exists; SceneRenderer hands the SRV over with the projection), gated by cloudShadowParams.x.
    pushSrv(cloudShadowSrv_.ptr != 0 ? cloudShadowSrv_ : fallbackSrv);
