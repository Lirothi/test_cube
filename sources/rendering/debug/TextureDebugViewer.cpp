...
namespace
{
    ...
    const char* TargetLabel(TextureDebugViewer::Target target)
    {
        ...
        case TextureDebugViewer::Target::LightShaftA: return "Light shafts A (mask)";
        case TextureDebugViewer::Target::LightShaftB: return "Light shafts B (final)";
        ...
        ID3D12Resource* oceanShoreDepth)
    {
        ...
        return {
            ...
            MakeTarget(TextureDebugViewer::Target::LightShaftA, "Lighting", "Plan A7 light shafts, half render resolution: the masked bloom SOURCE after the downsample (UE DownsampleLightShaftsPixelMain, bloom branch) -- bright sky and far pixels near the sun, thresholded, capped, faded at the screen edges; after the blur passes it also holds the second blur's output. Black everywhere = the pass did not run this frame (sun behind the camera, shafts off, or bloom scale 0; see the light shafts line in the session log).", D.lightShaftA.Get(), render::kLightShaftFormat),
            MakeTarget(TextureDebugViewer::Target::LightShaftB, "Lighting", "Plan A7 light shafts, half render resolution: the THIRD radial blur's output (UE BlurLightShaftsMain, 3 passes x 12 samples), which the apply adds into scene colour bilinearly. This is exactly what the shafts add to the frame.", D.lightShaftB.Get(), render::kLightShaftFormat),
