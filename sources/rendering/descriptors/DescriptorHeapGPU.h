...
public:
    ...
    // A6: a ring over [base, base + capacity) of a heap somebody else owns (render::BindlessHeap).
    // Handles come out with ABSOLUTE indices; GetHeap() is the shared heap.
    void InitView(ID3D12DescriptorHeap* heap, D3D12_DESCRIPTOR_HEAP_TYPE type, uint32_t base, uint32_t capacity, UINT incr) {
        device_ = nullptr;
        type_ = type;
        capacity_ = capacity;
        heap_ = heap;
        ...
        startCPU_.ptr += SIZE_T(base) * incr_;
        startGPU_ = heap->GetGPUDescriptorHandleForHeapStart();
        startGPU_.ptr += UINT64(base) * incr_;
        cursor_ = 0;
    }
...
private:
    ...
    uint32_t base_ = 0; // first absolute index of this ring inside heap_ (InitView)
